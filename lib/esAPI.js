...
function fetchQuery(query, endpoint, elastic, esoptions, fetchcallback) {
    ...
    client.query(query).execute(function(error, results){
        ...
        new esAPI(esoptions)
            .POST(elastic.real_index+"/"+elastic.type+"/_bulk", rows_str, default_callback("indexed " + results.results.bindings.length + " rows", false))
            .execute();
...
function applyAlias(elastic, esoptions){
    ...
    console.log('Current alias is ' + current_elastic.real_index);
    ...
    var res = request('GET', indexed_current_url)
    var count_next = JSON.parse(res.getBody('utf8')).count;
    if (isNaN(count_next)){
        count_next = 0;
    }
    ...
    // if new index size is > then old index of the five percent 
    if (increment_percent > -5){
        console.log('Alias is now ' + next_elastic.real_index);
        new esAPI(esoptions)
