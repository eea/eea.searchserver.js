...
const directory = '/data_downloads';
...
function indexWhenNoMoreOperations(opts){
    ...
    console.log("Waiting for transactions in ES to finish");
    ...
    try{
        ...
        var status = JSON.parse(res.getBody('utf8')).indices[opts.index].primaries.translog.uncommitted_operations;
        var docs_count = JSON.parse(res.getBody('utf8')).indices[opts.index].primaries.docs.count;
        if (docs_count === opts.offset){
            status = 0;
        }
        if (isNaN(status)){
            status = -1;
    ...
    if (status === 0){
        ...
        new esAPI(opts.esoptions)
            ...
            .execute();
        setTimeout(function(opts){opts.stream.resume()}, 100, opts);
    }
}

function fetchFileJSONLines(indexFile, esoptions, analyzers){
    var fs = require('fs');
    var elastic = nconf.get()['elastic'];
    elastic.real_index = elastic.index + "_blue";

    var stream = fs.createReadStream(indexFile.file, {flags: 'r', encoding: 'utf-8'});
    var buf = '';
...
function fetchFileJSON(indexFile, esoptions, analyzers){
    var fs = require('fs');
    var elastic = nconf.get()['elastic'];

    if (!elastic.increment_index_name){
        elastic.real_index = elastic.index + "_blue";
    }
    var inputStream = fs.createReadStream(indexFile.file, {flags: 'r', encoding: 'utf-8'});
    ...
    var stream = inputStream.pipe(JSONStream.parse(indexFile.data_path));
    stream.on('data', function(row) {
        var toindex = {};
        ...
        if (counter % bulk_size === 0){
            ...
            indexWhenNoMoreOperations({esoptions:esoptions, index:elastic.real_index, es_type:elastic.type, bulk:rows_str, stream:stream, offset:counter - bulk_counter});
            rows_str = "";
            bulk_counter = 0;
        }
        bulk_counter ++
...
function fetchFileCSV(indexFile, esoptions, analyzers){
    var fs = require('fs');
    ...
    var encoding = indexFile.encoding;
    ...
    var inputStream = fs.createReadStream(indexFile.file, encoding)
...
esAPI.prototype = {
    ...
    syncFromQuery: function(endpoint, queryTemplate, filtersQuery, analyzers) {
        ...
        if ((parent.options.encoded_auth !== undefined) && (parent.options.encoded_auth !== null) && (parent.options.encoded_auth.length > 0)){
            req_url = 'http://' + parent.options.encoded_auth + "@" + parent.options.es_host + '_aliases';
        }
        request(
            req_url,
    ...
    indexFromFile: function(indexFile, analyzers){
        ...
        if (!elastic.increment_index_name){
            elastic.real_index = elastic.index + "_blue";
            nconf.set ("elastic:real_index", elastic.real_index);
            var add_alias = '{"actions":[{"add":{"alias":"' + elastic.index + "_demo" +'","index":"'+elastic.real_index+'"}}]}';
            new esAPI(esoptions)
                .PUT(elastic.real_index, analyzers,
                    function(){
                        indexFile.format = indexFile.format || 'csv'
                        if (indexFile.format === 'csv'){
                            fetchFileCSV(indexFile, esoptions, analyzers);
                        }
                        if (indexFile.format === 'jsonlines'){
                            fetchFileJSONLines(indexFile, esoptions, analyzers);
                        }
                        if (indexFile.format === 'json'){
                            fetchFileJSON(indexFile, esoptions, analyzers);
                        }
                    })
            ...
            elastic.real_index = elastic.index + "_" + indexFile.time_stamp;
            nconf.set ("elastic:real_index", elastic.real_index);
            var remove_alias = '{"actions":[{"remove":{"alias":"' + elastic.demoalias + '","index":"*"}}]}';
            var add_alias = '{"actions":[{"add":{"alias":"' + elastic.demoalias +'","index":"'+elastic.real_index+'"}}]}';
            var add_generic_alias = '{"actions":[{"add":{"alias":"' + elastic.index +'","index":"'+elastic.real_index+'"}}]}';
            new esAPI(esoptions)
                .PUT(elastic.real_index, analyzers,
                    function(){
                        indexFile.format = indexFile.format || 'csv'
                ...
                .POST("_aliases", add_alias, default_callback(
                    "SUCCESS SWITCH ALIAS: Alias is now on " + elastic.real_index))
                .POST("_aliases", add_generic_alias, function(){
                    new esAPI(esoptions)
                        .GET(elastic.index + '*','',function(stats_err, stats_statusCode, stats_header, stats_body){
                            var stats = JSON.parse(stats_body);
                            var indexes = Object.keys(stats);
                            var idx_list = [];
                            for (var i = 0; i < indexes.length; i++){
                                if ((stats[indexes[i]].aliases[elastic.index] !== undefined) &&
                                    (stats[indexes[i]].aliases[elastic.demoalias] === undefined) &&
                                    (stats[indexes[i]].aliases[elastic.prodalias] === undefined)) {
                            ...
                            var threshold = elastic.increment_threshold || 3;
                            if (idx_list.length > threshold){
                                ...
                                new esAPI(esoptions)
                                    .DELETE(idx_to_delete.join(','), default_callback(
                                        "REMOVED OLD INDEXES: " + idx_to_delete.join(', ')))
                                    .execute();
                                ...
                                fs.readdir(directory, (err, files) => {
                                    ...
                                    for (const file of files) {
                                        for (const tmp_idx of idx_to_delete){
                                            if (file.startsWith(idx_to_delete)){
    ...
    testAnalyzers: function(analyzers, callback) {
        var esa = new esAPI(this.options);
    ...
    switchProdAlias: function(settings){
        var elastic = nconf.get()['elastic'];
        var esoptions = this.options;

        new esAPI(esoptions)
            .GET('_alias/' +  elastic.demoalias, '',
                function(rsp, rsp_code, rsp_header, rsp_data){
                    ...
                    for (var i = 0; i < indexes.length; i++){
                        ...
                        if (aliases !== undefined){
                            ...
                            for (var j = 0; j < alias_names.length; j++){
                                if (alias_names[j] === elastic.demoalias){
                    ...
                    if (found_indexes.length === 1){
                        var remove_alias = '{"actions":[{"remove":{"alias":"' + elastic.prodalias + '","index":"*"}}]}';
                        var add_alias = '{"actions":[{"add":{"alias":"' + elastic.prodalias + '","index":"' + found_indexes[0] + '"}}]}';
                        new esAPI(esoptions)
                            .POST("_aliases", remove_alias, default_callback(
                                "REMOVED OLD ALIAS"))
                            .POST("_aliases", add_alias, function(err, statusCode, header, body){
                                    ...
                                    default_callback("Alias " + elastic.prodalias + " points to " + found_indexes[0])(err, statusCode, header, body)
                                })
                            .execute();
                    }
                })
    ...
    indexesStatus: function(settings){
        var elastic = nconf.get()['elastic'];
        var esoptions = this.options;
        ...
        new esAPI(esoptions)
            .GET(elastic.index + '/_stats','',function(stats_err, stats_statusCode, stats_header, stats_body){
                new esAPI(esoptions)
                    .GET(elastic.prodalias,'',function(prod_err, prod_statusCode, prod_header, prod_body){
                        new esAPI(esoptions)
                            .GET(elastic.demoalias,'',function(demo_err, demo_statusCode, demo_header, demo_body){
                                var resp = {summary:{}, details:{}, fileInfo: fileInfo};
                                resp.details.stats = JSON.parse(stats_body)
                                resp.details.prod = JSON.parse(prod_body)
                                resp.details.demo = JSON.parse(demo_body)
                                resp.summary.prod = JSON.parse(prod_body)
                                resp.summary.demo = JSON.parse(demo_body)
                                if (resp.details.stats.error !== undefined){
                                    resp.details.stats.error = resp.details.stats.error.reason;
                                }
                                if (resp.details.prod.error !== undefined){
                                    resp.details.prod.error = resp.details.prod.error.reason;
                                }
                                if (resp.details.demo.error !== undefined){
                                    resp.details.demo.error = resp.details.demo.error.reason;
                                }

                                var prodIndexName = Object.keys(resp.summary.prod)[0];
                                delete resp.summary.prod[prodIndexName].settings;
                                if ((resp.details.stats.indices !== undefined) && (resp.details.stats.indices[prodIndexName] !== undefined)){
                                    resp.summary.prod[prodIndexName].docs = {};
                                    resp.summary.prod[prodIndexName].docs.count = resp.details.stats.indices[prodIndexName].primaries.docs.count;
                                    resp.summary.prod[prodIndexName].docs.uncommitted_operations = resp.details.stats.indices[prodIndexName].primaries.translog.uncommitted_operations;
                                }

                                var demoIndexName = Object.keys(resp.summary.demo)[0];
                                delete resp.summary.demo[demoIndexName].settings;
                                if ((resp.details.stats.indices !== undefined) && (resp.details.stats.indices[demoIndexName] !== undefined)){
                                    resp.summary.demo[demoIndexName].docs = {};
                                    resp.summary.demo[demoIndexName].docs.count = resp.details.stats.indices[demoIndexName].primaries.docs.count;
                                    resp.summary.demo[demoIndexName].docs.uncommitted_operations = resp.details.stats.indices[demoIndexName].primaries.translog.uncommitted_operations;
                                }
                                if (resp.summary.prod.error !== undefined){
                                    resp.summary.prod.error = resp.summary.prod.error.reason;
                                }
                                if (resp.summary.demo.error !== undefined){
                                    resp.summary.demo.error = resp.summary.demo.error.reason;
                                }

                                if (settings.API_callback !== undefined){
                                    settings.API_callback(resp)
                                }
                                else{
