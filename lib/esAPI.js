...
//}
    req.on('error', function(err) { console.log("XXXXXXXXXXXXXX");callback(err); });
    req.write(body);
    req.end();
};
...
console.log(esoptions);
    ...
    var indexed_current_url = esoptions.es_host+'/'+
                              current_elastic.real_index+'/_count';
    indexed_current_url = 'http://' + indexed_current_url.split("//").join("/");
    var indexed_next_url = esoptions.es_host+'/'+
                           next_elastic.real_index+'/_count';
    indexed_next_url = 'http://' + indexed_next_url.split("//").join("/");
    console.log(indexed_current_url);
    console.log(indexed_next_url);
    var count_current = 0;
    try{
...
console.log("try");
    ...
    if (increment_percent > SWITCH_CONDITION_VALUE_PERCENT){
        ...
        new esAPI(esoptions)
            .PUT("/_aliases", remove_alias, default_callback(
                "REMOVE OLD ALIAS: " + current_elastic.real_index))
            .PUT("/_aliases", add_alias, default_callback(
