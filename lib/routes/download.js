            ...
            if (!error && response.statusCode == 200) {
                try{
                    ...
                    if (!(es_index in data)){
                        ...
                        var es_alias_url = 'http://' + getAuth() + elasticConf.host + ':' + elasticConf.port + elasticConf.path + elasticConf.index + '/_alias'
                        var aliases = JSON.parse(sync_request('GET', es_alias_url ).getBody('utf8'));

                        if (aliases['error'] === undefined){
                            es_index = Object.keys(aliases)[0];
                        }
                        else {
                            console.log('error');
                            console.log(aliases['error'])
                            return;
                        }
                    }
