...
module.exports = function(req, res) {
    ...
    function download(mapping){
        ...
        var mapping_export = []
        for (var i = 0; i < mapping.fields_mapping.length; i++){
            var field = mapping.fields_mapping[i];
            if ((field.csv_tsv !== undefined) &&
                (field.csv_tsv.visible === true)){
                field.csv_tsv.name = field.name;
                mapping_export.push(field.csv_tsv);
            }
        }
        mapping_export.sort(sort_elements);
