            ...
            if (!error && response.statusCode == 200) {
                try{
                    ...
                    if (!(es_index in data)){
                        ...
                        if (elasticConf.increment_index_name){
                            var appalias = elasticConf.appalias || elasticConf.default_appalias;
                            var es_incr_alias_url = 'http://' + getAuth() + elastic_blue.host + ':' + elastic_blue.port + elastic_blue.path + appalias+'/_alias'
                            var incr_aliases = JSON.parse(sync_request('GET', es_incr_alias_url ).getBody('utf8'));
                    ...
                    var index_to_query = elastic.index;
                    if (elasticConf.increment_index_name){
                        index_to_query = es_index;
                    }
                    client.search({
