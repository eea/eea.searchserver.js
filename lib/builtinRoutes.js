...
var custom_nconf = nconf['stores']['custom'] || nconf['stores']['file'];
if (typeof layout_vars === 'undefined') {
    ...
    if (!layout_vars['override_searchserver_resources']) {
        ...
        layout_vars = custom_nconf['store']['layout_vars'];
        ...
        for (var i = 0, jlength = global_js_res_keys.length; i <= jlength; i++) {
            ...
            if (js_res && !js_res[global_js_res_key]) {
                ...
                js_res[global_js_res_key] = global_js_res[global_js_res_key].concat(js_res[global_js_res_key]);
            }
        }
        ...
        for (var j = 0, ilength = global_css_res_keys.length; j <= ilength; j++) {
            ...
            if (css_res && !css_res[global_css_res_key]) {
                ...
                css_res[global_css_res_key] = global_css_res[global_css_res_key].concat(css_res[global_css_res_key]);
            }
        }
    }
    else {
...
exports.index = async(req, res) => {
  ...
  await searchServer.EEAFacetFramework.render(req, res, 'index', options);
};
