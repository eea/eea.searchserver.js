function SimpleStart(settings){
  ...
  app.set('nconf', nconf);
  app.set('managementCommands', settings.managementCommands);
  ...
  app.get('/index', settings.routes.index);
  app.get('/details', settings.routes.details);
  ...
  searchServer.Server(app, settings.settingsFile, function(err, srv) {
    checkError(err);
    ...
    console.log("");
    srv.run(process.argv[2], settings.app_dir, process.argv.slice(3), function(err, srv) {
        checkError(err);
