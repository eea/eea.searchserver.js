function SimpleStart(settings){
  ...
  if (lv.url_folder){
    app.use(lv.url_folder, express.static(path.join(settings.app_dir, 'public')));
  }
  ...
  if (settings.customResourcesPath !== undefined ) {
      settings.customResourcesPath.forEach(function(dirpath) {
          ...
          if (lv.url_folder){
            app.use(path.join(lv.url_folder, "/app_resources"), express.static(dirpath));
            app.use(lv.url_folder, express.static(dirpath));
          }
  ...
  if (!lv.url_folder){
    ...
    app.get('/', function(req, res, next) {
      res.redirect(path.join(lv.url_folder,'index'));
    });
    app.get(lv.url_folder, function(req, res, next) {
      res.redirect(path.join(lv.url_folder,'index'));
    });
    app.get(path.join(lv.url_folder,'index'), settings.routes.routes.index);
