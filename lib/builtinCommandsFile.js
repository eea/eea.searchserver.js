...
var total_downloaded;
const proxy_pipe = new Transform({
    transform : function(chunk, encoding, done){
//        console.log(chunk.length);
        total_downloaded += chunk.length;
        console.log('downloaded:', total_downloaded);
        done(null, chunk);
    }
})

function updateStatusIndex(settings){
    var esAPI = require('eea-searchserver').esAPI;
    new esAPI(getOptions())
        .getStatus(function(status){
            _.extend(status, settings)
            new esAPI(getOptions())
                .setStatus(status)
        })
}
...
function createIndex(settings) {
    getIndexFiles(settings, function(config) {
      ...
      var indexName = elastic.index + "_" + time_stamp
      ...
      if ((tmp_indexFile.file === undefined) && (tmp_indexFile.url !== undefined)){
        updateStatusIndex({'filename':filename});
        ...
        .on('response', function(response){
            console.log('file_size:', response.headers['content-length']);
            updateStatusIndex({'filesize':response.headers['content-length']});
        })
        .pipe(proxy_pipe).pipe(tmp_file)
        .on('finish', function(){
