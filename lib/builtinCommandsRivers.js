...
function removeData(settings) {
    ...
    new esAPI(getOptions())
        ...
        .DELETE( elastic.index + "_blue_status",  callback('Deleting status index! (if it exists)') )
...
function getLastUpdateDate(elastic, cb){
    ...
    var request = require('sync-request');
    var indexed_url = 'http://' + getAuth('rw') + elastic.host + ':' + elastic.port + elastic.path +  elastic.index + '_status/last_update/_search?size=100';
    ...
    var options = {
        host: elastic.host,
        path: elastic.index + '_status/last_update/_search?size=1000',
        method: "GET",
...
function testMapping(settings, elastic,use_default_startTime,default_startTime,startTimeJson,clusters, syncIndex){
    var esAPI = require('eea-searchserver').esAPI;
    var river_configs = require('nconf').get()['river_configs'];
    ...
    // detecting indexes
    syncIndex = (syncIndex === null) ? false : syncIndex;
    ...
    .catch(function (reason) {
        ...
        elastic.other_index = elastic.index + "_green";

        new esAPI(getOptions())
            .testAnalyzers(config.analyzers, function(success){
                if (success){
                    startCreatingRiverClusters(settings, elastic,use_default_startTime,default_startTime,startTimeJson,clusters, true, false);
                }
            });
    })
    .then(function(res){
        elastic.real_index = res.real_index;
        elastic.other_index = res.other_index;
        new esAPI(getOptions())
            .testAnalyzers(config.analyzers, function(success){
                if (success){
                    startCreatingRiverClusters(settings, elastic,use_default_startTime,default_startTime,startTimeJson,clusters, false, syncIndex);
                }
            });
    });
...
function startCreatingRiverClusters(settings, elastic,use_default_startTime,default_startTime,startTimeJson,clusters,newIndex, syncIndex){
    var esAPI = require('eea-searchserver').esAPI;
    var river_configs = require('nconf').get()['river_configs'];
    var esQuery = new esAPI(getOptions());
    ...
    syncIndex = (syncIndex === null) ? false : syncIndex;
    newIndex = (newIndex === null) ? false : newIndex;

    // create eeariver index before adding them
    esQuery.PUT(getOptions().riverindex, {} , callback('Creating river index'));

    var firstConfig = null;

    // indexing river documents
    for (var i = 0; i < river_configs.configs.length; i++) {
        var cluster_id = river_configs.configs[i].id;

        // skip if cluster_id not in clusters

        if(clusters !== undefined && clusters !== null && clusters.length > 0 ){
            if(clusters.indexOf(cluster_id) === -1){
                continue;
            }
        }
        ...
        if (!use_default_startTime && startTimeJson[cluster_id]) {
            var updated_date = startTimeJson[cluster_id];
        ...
        if (shouldAdd){
            ...
            if(startTimeJson[cluster_id] === 0){
                river_last_update[cluster_id] = new Date(0).getTime();
            } else {
                river_last_update[cluster_id] = config.syncReq.eeaRDF.startTime;
            }


            if(elastic.real_index !== undefined && elastic.real_index !== null){
                config.syncReq.index.index = elastic.real_index;
                config.syncReq.index.statusIndex = elastic.real_index + "_status";
            } else {
                config.syncReq.index.statusIndex = elastic.index + "_status";
            }

            if(syncIndex){
                config.syncReq.index.switchAlias = true;
            }
            ...
            if(JSON.stringify(firstConfig) !==  JSON.stringify(config.analyzers) || firstConfig === null){
                //if new index create blue & green and alias _blue
                ...
                } else if(syncIndex && syncIndex != null){
                    var aC = {
                        "actions" : [
                            { "remove" : { "index" : elastic.real_index , "alias" : elastic.index } },
                        ]
                    };
                    esQuery.POST("_aliases", aC , callback('Deleting alias'));

                    var aQ = {
                        "actions" : [
                            { "add" : { "index" : elastic.other_index , "alias" : elastic.index } },
                        ]
                    };
                    esQuery.POST("_aliases", aQ , callback('Switching alias'));
            ...
            esQuery
                .DELETE( getOptions().riverindex + "/river/" + cluster_id, callback('Deleting river! (if it exists)'))
    ...
    if(syncIndex) {
        var aC = {
            "actions" : [
                { "remove" : { "index" : elastic.real_index + "_status", "alias" : elastic.index + "_status"} },
            ]
        };
        esQuery.POST("_aliases", aC , callback('Deleting alias'));
        ...
        var aQ = {
            "actions" : [
                { "add" : { "index" : elastic.other_index + "_status" , "alias" : elastic.index + "_status"} },
            ]
        };
        esQuery.POST("_aliases", aQ , callback('Switching alias'));
        ...
        Object.keys(river_last_update).forEach(function(rivK){
            ...
            if(lastDate === '1970-01-01T00:00:00'){
                lastDate = 0;
            }
            if(use_default_startTime) lastDate = 0;

            esQuery.PUT( elastic.other_index + '_status/last_update/' + rivK , {
                "name": rivK,
                "updated_at": lastDate
        ...
        if(newIndex){
            esQuery.PUT(elastic.index + '_blue_status', statusMappings, callback('Created status index'));

            esQuery.PUT(elastic.index + '_green_status', statusMappings, callback(''));
            ...
            Object.keys(river_last_update).forEach(function(rivK){
                ...
                esQuery.PUT( elastic.index + '_blue_status/last_update/' + rivK , {
                    "name": rivK,
                    "updated_at": lastDate
    ...
    /*esQuery.PUT(elastic.index + '/_status/last_update', {
...
function createIndex(settings, syncIndex, reindexing){
    ...
    if (settings.remove_all) {
        for (var i = 0; i < river_configs.configs.length; i++) {
            ...
            client.delete({
                index: getOptions().riverindex,
                type: 'river',
                id: river_configs.configs[i].id
            }, function (error, response) {
                // ...
                if(response) callback('Deleting river! (if it exists)');
        ...
        createBlueGreen(settings, function ( res ){
            ...
            getLastUpdateDate(elastic, function( rezult ){
                esQuery.POST( elastic.index + "/_delete_by_query", { "query" : { "match_all" : { }}}, callback('Deleting index! (if it exists)'));

                esQuery.POST( elastic.index +"_status" + "/_delete_by_query", { "query" : { "match_all" : { }}},
                    function(text,cb){
                        callback('Deleting status index! (if it exists)');

                        var t = {};
                        startTimeJson = Object.keys(rezult).map(function(item){
                            t[item] = 0;
                        });
                        startTimeJson = t;
                        ...
                        if ( startTimeJson !== -1 ) {
                            ...
                            compareElasticSemantic(settings, function (){
                                ...
                                testMapping(settings,elastic,use_default_startTime,default_startTime,startTimeJson,null, syncIndex);
                            });
                        } else {
                            if (use_default_startTime) {
                                console.log('Index objects newer than:', default_startTime);
        ...
        getLastUpdateDate(elastic, function( rezult ){
            ...
            if ( startTimeJson !== -1 ) {
                ...
                testMapping(settings,elastic,use_default_startTime,default_startTime,null,syncIndex);
            }
        });
    }
...
function createBlueGreen(settings, cb){
    ...
    function taskCb(tid, indexes){
        ...
        client.get({
            ...
            //TODO: reindex _status index
            console.log(indexes);
            var reindexStatus = {
                "source": {
                    "index": indexes.real_index + "_status",
                    "size": 1000
                },
                "dest": {
                    "index": indexes.other_index + "_status"
                }
            };
            console.log("Copying status index from :" + indexes.real_index + "_status" + " -> " + indexes.other_index + "_status" + " ...");
...
function syncIndex(settings){
    ...
    createBlueGreen(settings, function(res){
        ...
        if(res){
            createIndex(settings, res);
        }

    });

    //createIndex(settings, true);
}
...
function reCreateRivers(settings, clusters) {
    ...
    getLastUpdateDate(elastic, function(rez){
        ...
        startTimeJson = Object.keys(startTimeJson).map(function(item){
            var t = {};
            if(clusters.indexOf(item) !== -1){
                t[item] = 0;
                return t;
            } else {
                t[item] = startTimeJson[item];
                return t;
            }
        });

        testMapping(settings,elastic,use_default_startTime,default_startTime,startTimeJson,clusters,null, false);
...
function compareElasticSemantic(settings, cb) {
    ...
    getElasticData(function (err,elasticData) {
        if(semanticData !== undefined && elasticData !== undefined && semanticData.length > 0 && elasticData.length > 0) {
            semanticData.sort();
            elasticData.sort();
            ...
            //TODO: diffs activate - takes a long time
            //if(elasticData.length < semanticData.length){
                /*console.time("difffad");
                elasticData.forEach(function(val, ind, arr){
                    if(semanticData.indexOf(val) === -1){
           ...
           /* } else {
                ...
                if(diff !== undefined && diff.removed.length > 0) {
        ...
        } else if(elasticData === undefined || elasticData.length === 0){
            console.log("Elastic data not found...");
        } else {
...
/* Debugging example, how to test the deletion of a document from elastic
    ...
    try {
        ...
        //console.log("Got semantic list: " + semanticList.length);
...
function reIndex(settings) {
    ...
    createIndex(settings, true, true);
}
