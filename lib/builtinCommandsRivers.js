...
function removeRiver() {
    ...
    /*esQuery.GET(RIVER_INDEX + "/" + "river" + "/" + "_search",{}, callback('',function (er, body) {
        var res  = JSON.parse(body);
        if(res.hits !== undefined && res.hits.total === 0){
            esQuery.DELETE(RIVER_INDEX, callback("Deleted river index")).execute();
        }
    }));*/
    esQuery.execute();
}
...
function checkIfIndexing(settings, clusters) {
    ...
    /*if(clusters && Object.keys(clusters).length > 0){
        ...
        Object.keys(clusters).forEach(function(k){
            debugger;
            try {
                var res = request('GET', elastic.index + "_status" + "/" + "last_update" + "/" + clusters[k] + "", {
                    q: {},
                    'headers': {
                        //TODO :
                ...
                console.log("Couldn't get the status of cluster");
                console.log(e);
                debugger;
                process.exit(0);
            }
    ...
    for (var i = 0; i < river_configs.configs.length; i++) {
        ...
        if (shouldAdd){
            ...
            if(JSON.stringify(firstConfig) !==  JSON.stringify(config.analyzers) || firstConfig === null){
                esQuery
                    .POST(elastic.index , config.analyzers, callback('Setting up new index and analyzers'));

                firstConfig = JSON.stringify(config.analyzers);
            }
            esQuery
...
/*        if (e.statusCode === 403){
            console.log("FORBIDDEN!!!!" + e.message);
            return -1;
        }*/
...
function getElasticData(cb) {
    ...
    var client = new elasticsearch.Client({
        host: 'http://' + getAuth('rw') + elastic.host + ':' + elastic.port + elastic.path + elastic.index ,
        log: 'trace'
    });
    ...
    client.search({
        ...
        Object.keys(body.hits.hits).map(function(k){
            var kid = body.hits.hits[k];
            idList.push(kid._id);
        });
        if (body.hits.total > idList.length) {
...
function scrollIds(res_json2, idList){
