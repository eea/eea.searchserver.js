...
function checkIfIndexing(settings, clusters) {
    ...
    var esAPI = require('eea-searchserver').esAPI;
    var elastic = require('nconf').get()['elastic'];
    var rivers = 'http://' + elastic.host + ':' + elastic.port + elastic.path + RIVER_INDEX + '/_search';
...
function compareClusters(settings, args){
    ...
    try {
        debugger;
        var res = request('POST', elastic.index  + "/" +"_search" , {
            "json": aggsQ,
            'headers': {
                //TODO :
                'Authorization': "Basic "  + "dXNlcl9ydzpyd19wYXNz"
        ...
        console.log("Couldn't get the cluster ids. Getting from rivers confs");
        console.log(e);
