...
function startCreatingRiverClusters(settings, elastic,use_default_startTime,default_startTime,startTimeJson,clusters){
    ...
    if(elastic.real_index) {
        esQuery.PUT(elastic.real_index + '_status', statusMappings, callback('Created status index'));

        esQuery.PUT(elastic.other_index + '_status', statusMappings, callback(''));

        var qC = {
            "actions" : [
                { "add" : { "index" : elastic.real_index + "_status", "alias" : elastic.index + "_status" } }
            ]
        };
