                            ...
                            getDefaultDisplayFromSettings(function(settings_default_display){
                                middleware.templateRequired(req, res, function(){
                                    ...
                                    if (nconf.get("external_configs:defaults") !== undefined) {
                                        default_external_configs_file = path.join('/code/config/', APP_CONFIG_DIR, nconf.get("external_configs:defaults"));
                                    ...
                                    var merged_external_configs = _.extend(default_external_configs, external_configs);
                                    if (!_.isEmpty(merged_external_configs)){
                                        var req_path = req.url.split("?")[0];
                                        var req_path_parts = req_path.split("/");
                                        var req_location = req_path_parts[req_path_parts.length - 1];

                                        if (merged_external_configs[req_location] === undefined){
                                            merged_external_configs[req_location] = {};
                                        }

                                        options.external_config = merged_external_configs[req_location];
                                    }
                                    if (!landingCache){
                                        landingCache = {}
                                    }
