...
/*    app.get('/invalidate_templates', routes.invalidateTemplates);
    ...
    if (data_source !== undefined){
        if (data_source.type === 'url'){
            app.get('/API/v2/update_from_url', routes.API.update_from_url);
        }
    }

    if (nconf.get()["source"]["type"] === "url"){
...
function getIndexCreationDate() {
    ...
    var appalias = elastic.appalias || elastic.default_appalias;
    indexed_url = 'http://' + getAuth('rw') + elastic.host + ':' + elastic.port + elastic.path + elastic.index + '_' + appalias + '/_settings';
