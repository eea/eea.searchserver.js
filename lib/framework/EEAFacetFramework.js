...
function render(req, res, template, options, isDetail) {
  ...
  //  Get query language
  var default_language = nconf.get('default_language') ? nconf.get('default_language') : 'en'
  var languages = nconf.get('languages') ? nconf.get('languages') : { en: '' }
                        ...
                        getDisplayImagesFromSettings(function(settings_display_images){
                            getDefaultDisplayFromSettings(function(settings_default_display){
                                ...
                                middleware.externalConfigsRequired(req, res, function(){
                                    ...
                                    options.settings_default_language = "var settings_default_language = " + JSON.stringify(default_language) + ";";
                                    options.settings_languages = "var settings_languages = " + JSON.stringify(languages) + ";";
...
function getIndexCreationDate() {
    ...
    var indexed_url = 'http://' + getAuth('rw') + elastic.host + ':' + elastic.port + elastic.path + elastic.index + '/_settings';
    var creation_date = 'unknown';
    var res;

    try {
        res = request('GET', 'http://' + getAuth('rw') + elastic.host + ':' + elastic.port + elastic.path + elastic.index + "_status" +'/last_update/_search');
        var res_json = JSON.parse(res.getBody('utf8'));
        var hits = res_json.hits.hits;

        var min_update = hits.reduce(function(acc, cur){
        ...
        creation_date = dateFormat(min_update * 1000, 'dd mmmm yyyy HH:MM TT');
