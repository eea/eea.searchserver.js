...
/*    app.get('/invalidate_templates', routes.invalidateTemplates);
    ...
    app.get('/API/v1/full_reindex', routes.API.full_reindex);

    if ((nconf.get()["source"]["type"] === "url") || (nconf.get()["source"]["type"] === "file")){
        app.get('/API/v1/update_from_url', routes.API.update_from_url);
        if (nconf.get()["source"].configuration.sql){
            app.post('/API/v1/update_sql', routes.API.update_sql);
        }
    }
    ...
    if (lv_url_folder){
      app.get(path.join(lv_url_folder,'tools/api'), routes.elasticProxy);
      app.post(path.join(lv_url_folder,'tools/api'), routes.elasticProxy);
      app.use(lv_url_folder, express.static(path.join(__dirname, 'public')));
      app.use(path.join(lv_url_folder,"esbootstrap_resources"), express.static(path.join(__dirname, 'public')));
      app.get(path.join(lv_url_folder,'/tools/download'), routes.download);
    }
...
const render = async(req, res, template, options, isDetail) => {
  ...
  var autocorrect_disable = nconf.get()["autocorrect_disable"] || false;
  var autocomplete_include_searchterm = nconf.get()["autocomplete_include_searchterm"] || false;
  var es_debug_enabled = nconf.get()["es_debug_enabled"] || false;
  var es_version = nconf.get()['elastic']['version'] || "6";
  es_version = 'es'+es_version.trim();
                        ...
                        getDisplayImagesFromSettings(function(settings_display_images){
                            getDefaultDisplayFromSettings(function(settings_default_display){
                                ...
                                middleware.externalConfigsRequired(req, res, function(){
                                    ...
                                    options.settings_autocorrect_disable = "var settings_autocorrect_disable = " + autocorrect_disable + ";";
                                    options.settings_autocomplete_include_searchterm = "var settings_autocomplete_include_searchterm = " + autocomplete_include_searchterm + ";";
                                    ...
                                    options.settings_es_version = "var settings_es_version = '" + es_version + "';";
                                    ...
                                    var matomo_id = nconf.get("layout_vars")["matomo_id"] || false;
                                    options.settings_matomo_id = matomo_id
