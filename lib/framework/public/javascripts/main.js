...
const languages = window.settings_languages;
var langObj = {};
...
function translateChooselang() {
    ...
    for (var i = 0; i < languagesLinks.length; i++) {
        ...
        link.query = getQueryParams(link.queryString);

        if (lang !== default_language && languages[lang]) {
            link.query['lang'] = lang
        } else {
            link.query['lang'] = default_language
        }

        link.queryString = getQueryString(link.query)
...
window.jQuery(document).ready(function($){
    ...
    if ($.fn.Lazy) {
        $(window).on('results_ready', function() {
            $(".lazyLoad").Lazy();
            $.fn.i18nRender(langObj, langObj['lang']);
            if (!initialLoad && $('#landing').is(':hidden')) {
                $('#app').loading();
            }
            if (initialLoad) {
                initialLoad = false
