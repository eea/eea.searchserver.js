...
debugger;
        ...
        function fillMissingValuesWithExtremes(intervalInput, sliderInterval, groupedDataset, histogram_config){
            ...
            if(sliderRatio > 1){
                ...
                var tmaxArrG = fillMissingValues(null, (tmpArrMax[1] - tmpArrMax[0])/sliderRatio, tmpArrMax).map(Math.floor);

                var end = fillMissingValues(null, intervalInput/sliderRatio,
                    [ maxArrG[maxArrG.length-1], maxArrG[maxArrG.length-1]+parseFloat(intervalInput) ]).slice(1).map(Math.floor);

                var res = tminArrG.concat(result).concat(tmaxArrG).concat( end );
                debugger;
            } else {
                var end = [
