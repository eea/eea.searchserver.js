...
*/
        ...
        function groupDatasetByIntervalWithExtremes(set, intervalInput, histogram_config) {
            ...
            var result = groupDatasetByInterval(dataset, intervalInput).map(function(item, index, arr) {
                if (item.x !== undefined) {
                    if (!item.from || !item.to) {
                        ...
                        if(item.to === undefined){
                            item.to = arr[index + 1] !== undefined ? (arr[index + 1].x - 1).toFixed(dec) :
                                set_max.toFixed(dec);
                        }
                    } else {
            ...
            return minArrRes.concat(result).concat(maxArrRes);
        ...
        function fillMissingValuesWithExtremes(intervalInput, sliderInterval, groupedDataset, set, histogram_config){
            ...
            //if(sliderRatio > 1){
                ...
                /*var end = fillMissingValues(null, intervalInput/sliderRatio,
                    [ maxArrG[maxArrG.length-1], maxArrG[maxArrG.length-1] ]).slice(1).slice(0,-1).map(Math.floor);*/
            ...
            /*} else {
                ...
                debugger;
                var res = minArrG.concat(result).concat(maxArrG);
