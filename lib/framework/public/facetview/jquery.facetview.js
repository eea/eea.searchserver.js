                ...
                if ((jQuery.inArray(facet, options.aggregation_options) === -1)){
                    tmp_values.sort(function(a, b){
                        ...
                        if (translation && translation.enabled) {
                            ...
                            doc = new DOMParser().parseFromString('<div>' + a.text + '</div>', 'text/xml');
                            a_i18n_variable = langObj[langObj.lang][doc.querySelector('.i18n').getAttribute('i18n-variable')];
                            a_title = a_i18n_variable ? a_i18n_variable : a.facet_title;

                            doc = new DOMParser().parseFromString('<div>' + b.text + '</div>', 'text/xml');
                            b_i18n_variable = langObj[langObj.lang][doc.querySelector('.i18n').getAttribute('i18n-variable')];
        ...
        var dofacetgeo = function(rel, dothesearch) {
            ...
            if ($('div[id="facetview_group_' + relclean + '"]', obj).length) {
                ...
                var pobj = [
                            ...
                            `<span class="i18n" i18n-variable="Facet_Title(${title})" i18n-change="html" style="font-size: inherit">${title}</span>`,
                            '</h3>',
                            myobj,
                            '</div>'
                            ].join('');
