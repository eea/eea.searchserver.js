...
//                setfilterdescription(id, 'all');
        ...
        function createtreefromdata(tree, ord, opt, values) {
            ...
            // redo sort of tmp_values if we can get jstree to preserve the order
            // of tmp_values
            ...
            tree.jstree({
                ...
                'sort' : function(a, b) {
                    ...
                    var a_obj = data[a].original;
                    var b_obj = data[b].original;
                    if (ord === 'term') {
                        return a_obj.facet_title > b_obj.facet_title ? 1 : -1;
                    } else if (ord === 'reverse_term') {
                        return a_obj.facet_title > b_obj.facet_title ? -1 : 1;
                    } else {
                        if (ord === 'count') {
                            return a_obj.facet_count - b_obj.facet_count;
                        }
                        else {
                            return b_obj.facet_count - a_obj.facet_count;
                        }
                    }
                }
            })
