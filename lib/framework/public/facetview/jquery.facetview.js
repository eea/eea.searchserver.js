...
function checkLandingPage(options) {
    ...
    if (!remove_landing) {
        if ($('.facetview_filterselected').filter(':visible').length == 1) {
            var selectedFilterValue = $('.facetview_selection .facetview_filterselected').parent().text().trim(' ')
            for (var i = 0; i < facets.length; i++) {
                if (facets[i].default_values) {
                    for (var j = 0; j < facets[i].default_values.values.length; j++){
                        default_values.push(facets[i].default_values.values[j])
                    }
                }
            }
            if (default_values.indexOf(selectedFilterValue) > -1) {
                isLanding = true;
            } else {
                isLanding = false;
            }
        } else if ($('.facetview_filterselected').filter(':visible').length > 1) {
            isLanding = false;
        }
    }
