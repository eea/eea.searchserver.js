...
/*            options.paging.from = 0;
        ...
        function getInitialValuesInStorage(query, data){

            if(checkInitialSearch()){
                var hfacets = options.facets.filter(function (item){
                   return item.type === "rangehistogram";
                });

                $.each (hfacets, function(ki, item){
                    var d = data.facets[item.field].terms.map(function(term, k){
                        return {x: term.term, y: term.count };
                    });
...
//                window.history.pushState('', 'search', currurl);
            ...
            $.ajax({
                ...
                success: function(d){
                    getInitialValuesInStorage(removedisplaytype(qrystr), d);
                    showresults(d);
                } ,
                error: function(){
                    $(".facetview_top").hide();
