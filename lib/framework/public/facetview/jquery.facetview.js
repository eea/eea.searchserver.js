...
*/
        ...
        function groupDatasetByInterval(set, intervalInput) {
            ...
            var intervalRangeEnd = Math.floor( (set[ set.length-1 ].x / intervalInput) * intervalInput);

            for (var i = intervalRangeStart; i <= intervalRangeEnd; i += intervalInput) {
                var filt = set.filter( function(item){
                   return ( item.x >= i) && (item.x < i + intervalInput);
                });
                ...
                /*if(set[i].y < currentVal) {
                    result[result.length-1].x += set[i].x;
                } else if(set[i].y < currentVal + intervalInput){
                    result.push({y: currentVal, x: set[i].x});
                    currentVal += intervalInput;
                } else {
                    while(currentVal <= set[i].y) {
                        result.push({y: currentVal, x: 0});
                        currentVal += intervalInput;
                    }
                    result[result.length-1].x += set[i].x;
                }*/
        ...
        function facetRangeHistogram(event,  step) {
            ...
            intervalInput = histogram_config.columns_bucketsize || 10;
            ...
            renderSliderHistogram(rel, event, step, datasetFilledMissing, datasetGroupByInterval, intervalInput);
            setTimeout(function(){
        ...
        function renderHistogram(rel, dataset, intervalInput, event) {
            ...
            var histoDatasetLength = histoDataset.length - 1;
            var facet = $(event.target).attr('id');
            var facetOptions = options.facets.filter(function(f){
                return f.field === facet;
            })[0];
            ...
            var datasetLength = histoDataset.length-1;
            svg.selectAll("rect")
                .data( histoDataset )
                .enter()
                .append("rect")
                ...
                .attr("width", Math.abs(width / datasetLength  - padding))
                ...
                .on("mouseover", function(d) {
                    tooltip.transition()
                        .duration(200);
                    tooltip.html( "<p><strong>" + d.y + " - " + (d.y + intervalInput - 1) + "</strong></p><p><span>Number: " + d.x + "</span></p>")
        ...
        function fillMissingValues(set, step, intervalInput) {

            var result = [getFirstValue(set[0])]; // reset to 0 if for example interval starts at 1953
            var currentVal = result[0] + step;
            ...
            if(result.length % intervalInput !== 0) {
                while(currentVal <= lastValue + intervalInput - step) {
                    result.push(currentVal);
                    currentVal += step;
                }
            }
        ...
        function renderSliderHistogram(rel, event, step, dataset, groupedDataset, intervalInput) {
            ...
            //var values = fillMissingValues(valsobj.data("values"), step, intervalInput);
            var values = groupedDataset.map(function(item){ return item.x});
            var min = 0;
            var max = values.length - 1;
