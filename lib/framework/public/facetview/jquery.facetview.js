...
//                setfilterdescription("facetview_group_" + relclean, op_text);
        ...
        var clearfilter = function(event) {
            ...
            options.paging.from = 0;
            if (!children_clicked) {
                options.from_postate = true;
            }
            dosearch();
            if (!children_clicked) {
                options.from_postate = false;
            }
        };
            ...
            for (var each = 0; each < options.facets.length; each++) {
                ...
                if ($.inArray(facet, options.rangefacets) !== -1){
                    ...
                    function parseRangeFacetValues(sdata){
                        ...
                        if (header_class.indexOf('facetview_open') === -1 && header_class.indexOf('exact_helper') === -1) {
                           $header.click(); 
                        }
                        correctFacetRenderer();
                    }
...
$(document).ready(function($){
    ...
    window.onpopstate = function(event) {
        ...
        if ([-1, 1, 0].indexOf(queries_change) !== -1 && url_filters.length - filters.length === 0) {
