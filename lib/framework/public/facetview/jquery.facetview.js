...
*/
        ...
        function groupDatasetByIntervalWithExtremes(set, intervalInput, histogram_config) {
            var minArr = set.filter(function (item, index) {
                return item.x < histogram_config.columns_bucket_min;
            });

            var maxArr = set.filter(function (item, index) {
                return item.x > histogram_config.columns_bucket_max;
            });

            var dataset = set.filter(function (item, index) {
                return item.x >= histogram_config.columns_bucket_min && item.x <= histogram_config.columns_bucket_max;
            });
        ...
        function fillMissingValues(set, step, intervalInput, sliderInterval, groupedDataset) {
            ...
            var intervalRangeStart = groupedDataset[0].x;
            var intervalRangeEnd = groupedDataset[groupedDataset.length - 1].x;
