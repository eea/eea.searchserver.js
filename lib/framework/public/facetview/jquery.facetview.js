...
function checkLandingPage(options) {
    ...
    if (!remove_landing) {
        var selected_values = {};
        $(".facetview_selected").each(function(id, element) {
            var facet_name = $(element).attr('id').substr(16);
            selected_values[facet_name] = [];
            $(element).find(".facetview_filterselected").each(function(s_id, selected_element) {
                selected_values[facet_name].push($(selected_element).attr("href"));
            });
        });
        let ok = true
        let default_values_length = 0
        for (var i = 0; i < facets.length; i++) {
            if (facets[i].default_values) {
                let facetName = facets[i].name
                let default_values = facets[i].default_values.values
                ok = selected_values.hasOwnProperty(facetName)
                if (ok && default_values.length === selected_values[facetName].length ) {
                    default_values_length++
                    ok = default_values.every(function(value){
                        return selected_values[facetName].indexOf(value) !== -1
                    })
                } else {
                    ok = false
                }
        ...
        if (default_values_length !== Object.keys(selected_values).length) {
            ok = false
        }
        if (!ok) {
            isLanding = false
...
function findObjectsByKey(obj, key) {
        let list = [ ];
        if (!obj) return list;
        if (obj instanceof Array) {
            for (var i in obj) {
                list = list.concat(this.findObjectsByKey(obj[i], key));
            }
            return list;
        }
        ...
        if ((typeof obj == "object") && (obj !== null)) {
            ...
            if (children.length > 0) {
                for (let i = 0; i < children.length; i++) {
                    list = list.concat(this.findObjectsByKey(obj[children[i]], key));
                }
            }
        }
        return list;
...
function customSetUrl(options, stateObj, page, url){
    ...
    newUrl.queryString = url;
    newUrl.queryParams = getQueryParams(newUrl.queryString);

    query = getQueryParams(window.location.search);

    if (translation && translation.enabled) {
        default_language = translation.default_language;
        languages = translation.languages;
        //  Put lang parameter in query
        if (getQueryLang(query) === default_language) {
            if (getQueryLang(newUrl.queryParams)) {
                delete newUrl.queryParams['lang'];
            }
        } else if (getQueryLang(query)){
            if (getQueryLang(newUrl.queryParams) && languages[getQueryLang(query)]) {
                newUrl.queryParams['lang'] = getQueryLang(query);
            } else if (languages[getQueryLang(query)]) {
                newUrl.queryParams = Object.assign({
                    lang: getQueryLang(query)
                }, newUrl.queryParams);
            }
        }
    }
    newUrl.queryString = getQueryString(newUrl.queryParams);

    url = newUrl.queryString;

    if (window.isInDebugMode){
        url = "?debug=true&" + url.substr(1);
    }
...
jQuery.extend({
    getUrlVars: function(options) {
        ...
        if ((params.debug !== undefined) && (params.debug.toLowerCase() === "true")){
            window.isInDebugMode = true;
        }
...
search_text_input_clear
----------------
An option to include a X icon in input text in order to clear the search box
If not set will be False by default

settings_autocorrect_disable
----------------
An option to disable the search on autocorrected terms
If not set will be False by default

settings_autocomplete_include_searchterm
----------------
An option to include the search term in the autocomplete menu generated with
jqueryui as the first item
If not set will be False by default
...
(function($) {
    ...
    $.fn.facetview = function(options) {
        ...
        $.fn.facetview.provided_options = $.extend(true, {}, provided_options);
        ...
        if (options.rangehistogramfacets.length > 0){
            var rangeminmaxquery = {
                "size": 0,
                "aggs": {}};
            for (var i = 0; i < options.rangehistogramfacets.length; i++){
                var use_field = true;
                for (var j = 0; j < options.facets.length; j++){
                    if ((options.facets[j].field === options.rangehistogramfacets[i]) && (options.facets[j].histogram_config.min_max_script !== undefined)){
                        use_field = false;
                        rangeminmaxquery.aggs["min_" + options.rangehistogramfacets[i]] = {"min": {"script": options.facets[j].histogram_config.min_max_script}};
                        rangeminmaxquery.aggs["max_" + options.rangehistogramfacets[i]] = {"max": {"script": options.facets[j].histogram_config.min_max_script}};

                    }
                }
                if (use_field){
                    rangeminmaxquery.aggs["min_" + options.rangehistogramfacets[i]] = {"min": {"field": options.rangehistogramfacets[i]}};
                    rangeminmaxquery.aggs["max_" + options.rangehistogramfacets[i]] = {"max": {"field": options.rangehistogramfacets[i]}};
                }
            };
            if ((options.predefined_filters) && (!jQuery.isEmptyObject(options.predefined_filters))){
                rangeminmaxquery.query = {"bool":{"must":options.predefined_filters}};
            }
            $.ajax({
                contentType: "application/json",
                type: 'post',
                url: options.search_url,
                data: JSON.stringify({source: JSON.stringify(rangeminmaxquery)}),
                ...
                success:function(d){
                    ...
                    for (var i = 0; i < options.facets.length; i++){
                        if ((options.facets[i].histogram_config !== undefined) && (options.facets[i].histogram_config !== null) && (options.facets[i].type === 'rangehistogram')){
                            var histogram_config = options.facets[i].histogram_config
                            var cbs_decimals = countDecimals(histogram_config.columns_bucketsize || 0)
                            var fname = options.facets[i].field;
                            histogram_config.min_val = Math.min(d.aggregations["min_" + fname].value, histogram_config.columns_bucket_min || d.aggregations["min_" + fname].value);
                            histogram_config.max_val = Math.max(d.aggregations["max_" + fname].value, histogram_config.columns_bucket_max || d.aggregations["max_" + fname].value);
                            histogram_config.ranges = [];
                            var column_buckets_min = histogram_config.columns_bucket_min;
                            var column_buckets_max = histogram_config.columns_bucket_max;
                            if (histogram_config.histogram){
                                if (column_buckets_min === undefined){
                                    column_buckets_min = parseFloat((Math.floor(histogram_config.min_val / histogram_config.columns_bucketsize) * histogram_config.columns_bucketsize).toFixed(cbs_decimals));
                                }
                                else {
                                    histogram_config.ranges.push({"to": column_buckets_min});
                                }
                                if (column_buckets_max === undefined){
                                    column_buckets_max = parseFloat(((Math.floor(histogram_config.max_val / histogram_config.columns_bucketsize) + 1) * histogram_config.columns_bucketsize).toFixed(cbs_decimals));
                                }
                                else {
                                    histogram_config.ranges.push({"from": column_buckets_max});
                                }
                                for (var j = column_buckets_min; j < column_buckets_max; j += histogram_config.columns_bucketsize){
                                    histogram_config.ranges.push({"from": j, "to": j + histogram_config.columns_bucketsize});
                                }
                            }
                            else {
                                histogram_config.ranges.push({"from": column_buckets_min, "to": column_buckets_max});
                            }
                        }
                    }
                },
        ...
        // check url_options, if it only has a 'q' and/or 'lang' paramaters, handle it as an initial search, and apply all default settings
        var optionsToIgnore = 0;
        if (url_options.q !== undefined){
            optionsToIgnore++;
        }
        if (url_options.lang !== undefined){
            optionsToIgnore++;
        }
        if ((Object.keys(url_options).length === optionsToIgnore)){
            options.isInitialSearch = true;
        }
...
*/
        ...
        function createtreefromdata(tree, ord, opt, values) {
            ...
            var facet_config = options.facets[options.facetids[facet]];
            var facet_size = 10;
            var facet_whitelist = [];
            var facet_blacklist = [];
            ...
            if ((jQuery.inArray(facet, options.rangefacets) !== -1) || (jQuery.inArray(facet, options.rangehistogramfacets) !== -1)){
                ...
                if (jQuery.inArray(facet, options.aggregation_options) !== -1){
                    // if radio facet, build the specific items, and add the 'All' option in top
                    if (values.length > 0){
                        var vals_total = 0;
                        for (var val_cnt = 0; val_cnt < values.length; val_cnt++){
                            vals_total += parseInt($("<div>" + values[val_cnt].text + "</div>").find(".facet_label_count").text())
                        }
                        var option_all = {
                            text: "<span class='facet_label_count'>" + vals_total + "</span><span class='facet_label_text i18n' i18n-variable='Facet_" + facet.charAt(0).toUpperCase() + facet.slice(1) + "_Item(All)' i18n-change='html'>All</span>",
                            li_attr:{
                                class: values[0].li_attr.class,
                                rel: values[0].li_attr.rel,
                                title: "All"
                            }
                        }
                ...
                for (var bvalue_idx = 0; bvalue_idx < bvalue_length; bvalue_idx++){
                    ...
                    tmp_values.push(blacklisted_values[bvalue_idx]);
                }
                if ((jQuery.inArray(facet, options.aggregation_options) === -1)){
                    tmp_values = nonzero_values;
                }
            }

            if ('fixed_order' in current_facet) {
                var tmp_order = [];
                var extra_items = [];
                for (var dvalue_idx = 0; dvalue_idx < tmp_values.length; dvalue_idx++) {
                    if (jQuery.inArray(tmp_values[dvalue_idx].facet_title, current_facet.fixed_order) !== -1) {
                        tmp_order[current_facet.fixed_order.indexOf(tmp_values[dvalue_idx].facet_title)] = tmp_values[dvalue_idx];
                    }
                    else {
                        extra_items.push(tmp_values[dvalue_idx])
                    }
                }

                var filtered_order = tmp_order.filter(function(el) { return el; });
                tmp_values = filtered_order.concat(extra_items);
            }
            else {
                ...
                if ((jQuery.inArray(facet, options.aggregation_options) === -1)){
                    tmp_values.sort(function(a, b){
                        var a_title, b_title;
                        a_title = a.facet_title;
                        b_title = b.facet_title;
                        if (translation && translation.enabled) {
                            var doc, a_i18n_variable, b_i18n_variable;

                            doc = $("<div>" + a.text + "</div>");
                            a_i18n_variable = langObj[langObj.lang][doc.find(".i18n").attr("i18n-variable")];
                            a_title = a_i18n_variable ? a_i18n_variable : a.facet_title;

                            doc = $("<div>" + b.text + "</div>");
                            b_i18n_variable = langObj[langObj.lang][doc.find(".i18n").attr("i18n-variable")];
                            b_title = b_i18n_variable ? b_i18n_variable : b.facet_title;
                        }


                        if (a_title === facet_config.first_value) {
                          return -1;
                        }
                        if (b_title === facet_config.first_value) {
                          return 1;
                        }

                        if (a_title === facet_config.last_value) {
                          return 1;
                        }
                        if (b_title === facet_config.last_value) {
                          return -1;
                        }
                    ...
                    // if radio facet, load the order from the configuration file
                    for (var f_cnt = 0; f_cnt < options.facets.length; f_cnt++){
                        if (options.facets[f_cnt].field === facet){
                            var tmp2_values = [tmp_values[0]];
                            for (var agg_cnt = 0; agg_cnt < options.facets[f_cnt].aggregation_options.length; agg_cnt++){
                                for (var tmpf_cnt = 0; tmpf_cnt < tmp_values.length; tmpf_cnt++){
                                    if (options.facets[f_cnt].aggregation_options[agg_cnt].label === tmp_values[tmpf_cnt].facet_title){
                                        tmp2_values.push(tmp_values[tmpf_cnt]);
                                    }
                                }
                            }
                            tmp_values = tmp2_values;
                        }
                    }
                }
            }
            ...
            if (tree.jstree && tmp_values.length) {
                ...
                .bind('select_node.jstree', function(event, data) {
                    ...
                    if (attributes.class.indexOf('leaf') > -1) {
                        ...
                        $.fn.facetview.correct_and_dosearch(opt);
                    } else {
            ...
            if (jQuery.inArray(facet, options.aggregation_options) !== -1){
                addRadioOptions(facet, tree);
            }
            correctFacetRenderer();
        }

        var addRadioOptions = function(facet, tree) {
            tree.find(".jstree-anchor").append('<span class="eea-tree-radiobutton"></span>');
            var selected = $("#facetview_group_" + facet).find(".facetview_filterselected").attr("href") || "All";
            tree.find("[title='" + selected + "']").find(".eea-tree-radiobutton").addClass("ui-icon-check").addClass("ui-icon");
        }
        ...
        var sortfilters = function(event) {
            ...
            if (facet.size >= Object.keys(values).length){
                ...
                $filters.sort(function(a,b){
                    ...
                    b_title = $(b).attr("title");
                    if (translation && translation.enabled) {
                        var a_i18n_variable, b_i18n_variable;

                        a_i18n_variable = langObj[langObj.lang][a.querySelector('.i18n').getAttribute('i18n-variable')];
                        a_title = a_i18n_variable ? a_i18n_variable : $(a).attr("title");

                        b_i18n_variable = langObj[langObj.lang][b.querySelector('.i18n').getAttribute('i18n-variable')];
                        b_title = b_i18n_variable ? b_i18n_variable : $(b).attr("title");
                    }

                    if (a_title === facet.first_value) {
                      return -1;
                    }
                    if (b_title === facet.first_value) {
                      return 1;
                    }

                    if (a_title === facet.last_value) {
                      return 1;
                    }
                    if (b_title === facet.last_value) {
                      return -1;
                    }
        ...
        var dofacetgeo = function(rel, dothesearch) {
            ...
            var myobj = '<div class="facetview_selection facetview_selection1"> <a ' +
            ...
            if ($('div[id="facetview_group_' + relclean + '"]', obj).length) {
                ...
                var pobj = [
                            ...
                            '<h3 class="facetview_group_title facetview_group_title1">',
                            `<span class="i18n title" i18n-variable="Facet_Title(${title})" i18n-change="html" style="font-size: inherit">${title}</span>`,
                            '</h3>',
                            myobj,
                            '</div>'
                            ].join('');
        ...
        var dofacetrange = function(rel, use_html_values) {
            ...
            if(values !== undefined && values.length > 0){
                if (parseFloat(min) === values[0] && parseFloat(max) === values[values.length - 1]){
                    ...
                    if ($('.facetview_selected').length === 0) {
                        if (!options.current_filters_show){
                            $('.current-filters').hide();
                        }
                    }
            ...
            var myobj = '<div class="facetview_selection facetview_selection2"> <a ' +
            ...
            if ($('div[id="facetview_group_' + relclean + '"]', obj).length) {
                ...
                var pobj = [
                            ...
                            '<h3 class="facetview_group_title facetview_group_title2">',
                            `<span class="i18n title" i18n-variable="Facet_Title(${title})" i18n-change="html" style="font-size: inherit">${title}</span>`,
                            '</h3>',
                            myobj,
                            '</div>'
        ...
        var dofacetrangeHistogram = function(rel, use_html_values, histogram_config, chosenValues) {
            ...
            var myobj = '<div class="facetview_selection facetview_selection3">';
            ...
            myobj = [myobj,
                ...
                highval,
                '</span>',
                '<a class="facetview_filterselected facetview_clear btn',
                '" ',
                'facettype="rangehistogram"',
                ' rel="',
                rel,
            ...
            if ($('div[id="facetview_group_' + relclean + '"]', obj).length) {
                ...
                var pobj = [
                    ...
                    relclean,
                    '" class="btn-group facetview_selected ' + exact_helper_class + '">',
                    '<h3 class="facetview_group_title facetview_group_title3">',
                    `<span class="i18n title" i18n-variable="Facet_Title(${title})" i18n-change="html" style="font-size: inherit">${title}:</span>`,
                    '</h3>',
                    myobj,
                    '</div>'
        ...
        var createfacetgeo = function (options){
            ...
            if (gmaps_loaded) {
                ...
                $.each(eea_mapping.facets, function (key, facet_loop) {
                    if (facet_loop.name === facet){
                        if (facet_loop.default_bounds !== undefined) {
                            default_bounds = facet_loop.default_bounds;
                        } else {
                            default_bounds = {
                                "lat1": 50.738525,
                                "lng1": 9.981955,
                        ...
                        if (facet_loop.default_distance !== undefined) {
                            default_distance = facet_loop.default_distance;
                        } else {
                            default_distance = {
                                "lat": 48.738525,
                                "lng": 13.981955,
        ...
        //   ]
        function groupDatasetByInterval(dset, intervalInput,set_min, set_max, discrete) {
        ...
        function groupDatasetByIntervalWithExtremes(set, intervalInput, histogram_config) {
            if (!histogram_config.histogram){
                return [{"from":set[0].x, "to": set[set.length-1].x, "x": set[0].x}];
            }
...
//                var set_max_I = set_max + intervalInput;
        ...
        function formatRecords(records, histogram_config){
            var formatedRecords = [];
            for (var record_cnt = 0; record_cnt < Object.keys(records).length; record_cnt++){
                var key = Object.keys(records)[record_cnt];
                var tmp_from = key.split('-')[0];
                var tmp_to = key.split('-')[1];
                if (tmp_from === "*"){
                    tmp_from = histogram_config.min_val;
                }
                else {
                    tmp_from = parseFloat(tmp_from)
                }
                if (tmp_to === "*"){
                    tmp_to = histogram_config.max_val;
                }
                else {
                    tmp_to = parseFloat(tmp_to)
                }
                var rec_x = tmp_from;
                var rec_y = records[key];
                var rec_from = tmp_from.toString();
                var rec_to = tmp_to.toString();
                var data_record = {
                        x: rec_x,
                        y: rec_y,
                        from: rec_from,
                ...
                formatedRecords.push(data_record);
            }
        ...
        function facetRangeHistogram(event,  step) {
            ...
            var datasetGroupByInterval = 0;
            if(valsobj.data("group_values") === undefined) {
                datasetGroupByInterval = formatRecords(valsobj.data().records, histogram_config);
            } else {
                datasetGroupByInterval = formatRecords(valsobj.data("group_values"), histogram_config);
            }

            var datasetFilledMissing = []

            if (histogram_config.discrete_values){
                for (var i = 0; i < datasetGroupByInterval.length; i++){
                    if (parseFloat(datasetGroupByInterval[i].to) - parseFloat(datasetGroupByInterval[i].from) <= histogram_config.columns_bucketsize){
                        datasetGroupByInterval[i].to = datasetGroupByInterval[i].from;
                    }
                }
            }
//createValuesObj(valsobj.data("group_values"));
/*
            ...
            if( sessionStorage.getItem(GROUPS_KEY + rel) !== null){
                ...
                var valsFromSession = sessionData.aggregations[options.facets[rel].field];
...
/*            if(histoDataset.length === 0){
        ...
        function renderSliderHistogram(rel, event, step, dataset, groupedDataset, intervalInput, sliderInterval, histogram_config,
            ...
            var values = fillMissingValuesWithExtremes(intervalInput, sliderInterval, groupedDataset, groupedDataset, histogram_config);
            ...
            var mu = muT === undefined ? "" : `<span class="um i18n" i18n-variable="App_Word(${muT})" i18n-change="html">${muT}</span>`;
            var rangeselect = [
                '<div id="facetview_rangeplaceholder_',
                rel,
...
/*                    if ( options.enable_rangeselect ) {
                ...
                if (allow_exact){
                    ...
                    exact_html =
                        '<div class="exact_facet_helper">'+
                            '<span class="eea-icon eea-icon-right eea-icon-question-circle eea-flexible-tooltip-top i18n"'+
                                'title="The *Exact* option will give you results that contain exclusivly the selected values and nothing else. This option may give you more precise results. Try it to find out." i18n-variable="Tooltip(exact)" i18n-change="title">'+
                            '</span>'+
                            '<span class="i18n" i18n-variable="App_Word(Exact)" i18n-change="html">Exact:</span>'+
                    ...
                    autocomplete_html =
                        '<div class="facetview_filter_typeahead">'+
                            '<div>'+
                                ...
                                '<input class="i18n" i18n-variable="Facet_' + autocomplete_placeholder.split(' ').join('_') + '" i18n-change="placeholder" placeholder="' + autocomplete_placeholder + '" type="text" rel="' + prop +  '"/>' +
                            '</div>'+
                ...
                var facet_options = '';
                var option_group_class = '';
                if (facet.type !== 'aggregation_options'){
                    //disable the sorting, exact, and/or functionality if radio facet
                    facet_options = [
                        '<div class="btn-group facetview_filter_options" ',
                        'style="display:none;">',
                        '<div class="facet_order facet_count_order ' + myOrder['count_class'] + '"><span class="i18n" i18n-variable="App_Word(Count)" i18n-change="html">Count</span><span class="eea-icon ' + myOrder['count_icon'] + '"></span></div>',
                        '<div class="facet_order facet_value_order ' + myOrder['term_class'] + '">' + '<span class="i18n" i18n-variable="Facet_Sortby(' + short_name + ')" i18n-change="html">' + short_name + '</span>' + '<span class="eea-icon ' + myOrder['term_icon'] + '"></span></div>',
                        ...
                        '<select class="facet_value_operator">',
                            '<option class="i18n" i18n-variable="App_Sentence(Match all)" i18n-change="html" value="and"' + option_rel_all_selected + '>Match all</option>',
                            '<option class="i18n" i18n-variable="App_Sentence(Match any)" i18n-change="html" value="or"' + option_rel_any_selected + '>Match any</option>',
                        '<select>',
                        '</div>',
                        '</div>'
                    ].join('')
                    ...
                    option_group_class = 'option_group'
                }
                html = [html,
                        '<div class="facetview_filter"> <h2 ',
                        'class="facetview_showtree ' + exact_helper_class + ' i18n" title="',
                        valuetext,
                        '" i18n-variable="Facet_Title(',
                        valuetext,
                        ')" i18n-change="title" id="',
                        prop,
                        '" eea_rel="',
                        idx,
                        '">',
                        '<div class="facetview_showtree_eealabel i18n" i18n-variable="Facet_Title(',
                        valuetext,
                        ')" i18n-change="html">',
                        valuetext,
                        '</div>',
                        ...
                        facet_options,
                        empty_html,
                        '<div class="facetview_tree ' + option_group_class + '" style="display:none;',
                ...
                autocomplete_input.autocompleteForFacet(
                    {source:function(request, response){
                        ...
                        select:function(event, ui){
                            selectFromAutocomplete(event, ui.item.rel, ui.item.originalTerm);
                        }
                    });
                }
            }
        ...
        $.fn.facetview.clickfilterchoice = function(event, rel, href, initiator) {
            ...
            var resetOptionsFacet = false;
            var isOptionsFacet = false;
            if ($.inArray(rel, options.aggregation_options) !== -1){
                isOptionsFacet = true;
                if (href === 'All'){
                    $('a.facetview_filterselected[rel="' + rel + '"]').closest(".facetview_selected").remove();
                    resetOptionsFacet = true;
                }
                else {
                    $('a.facetview_filterselected[rel="' + rel + '"]').closest(".facetview_selection").remove();
                }
                var tree = $('.facetview_tree[rel="' + rel + '"]');
                tree.find(".eea-tree-radiobutton").removeClass("ui-icon-check").removeClass("ui-icon");
                tree.find(".facetview_filterchoice[title='"+href+"']").find(".eea-tree-radiobutton").addClass("ui-icon-check").addClass("ui-icon");
            }

            if (!resetOptionsFacet){
                var myobj = '<div class="facetview_selection">';
                var operation = $('.facetview_or[href="' + rel + '"]', obj);
                var op_text = 'all';
                var a_extra_class = ''
                if (operation.attr('rel') === 'OR' || initiator) {
                    a_extra_class = ' facetview_logic_or';
                    op_text = 'any';
                }
                myobj = [myobj,
                     `<span class="i18n" i18n-variable="Facet_${rel.charAt(0).toUpperCase() + rel.slice(1)}_Item(${href})" i18n-change="html">`,
                     href,
                     '</span>',
                     '<a class="facetview_filterselected facetview_clear btn',
                     a_extra_class,
                ...
                if ($('div[id="facetview_group_' + relclean + '"]', obj).length) {
                    ...
                    var pobj = [
                            ...
                            relclean,
                            '" class="btn-group facetview_selected">',
                            '<h3 class="facetview_group_title facetview_group_title4">',
                            `<span class="i18n title" i18n-variable="Facet_Title(${title})" i18n-change="html" style="font-size: inherit">${title}</span>`,
                            !isOptionsFacet ?'<span> (in ' + op_text + '):</span>':"",
                            '</h3>',
                            myobj,
                            '</div>'
                            ].join('');
    ...
    //                setfilterdescription("facetview_group_" + relclean, op_text);
        ...
        var parsefacet = function(facet) {
            var facetsobj = {};
            for (var thing = 0; thing < facet.buckets.length; thing++) {
                facetsobj[facet.buckets[thing].key] = facet.buckets[thing].doc_count;
            }
        ...
        function findElemInExplanation(field, type, explanation){
            var str_to_find = ""
            if (type === "decay"){
                str_to_find = "Function for field " + field;
            }
            if (type === "script_score"){
                str_to_find = field;
            }
            if (type === "field_boost"){
                str_to_find = "weight("+field + ":";
            }
            if (explanation.description.includes(str_to_find)){
                return (explanation.value);
            }

            for (var tmp_cnt = 0; tmp_cnt < explanation.details.length; tmp_cnt++){
                detail = explanation.details[tmp_cnt];
                var val = findElemInExplanation(field, type, detail);
                if (val !== 0){
                    return val;
                }
            }
        ...
        function extractFormula(explanation, path){
            var id = explanation.description.replace(/[^\w]/gi, '')
            if (path !== "#"){
                id = path + "_" + explanation.description.replace(/[^\w]/gi, '')
            }
            var tmp_ret = [];
            var obj = {"id": id,
                        "parent":path,
                        "text": explanation.value + " (" + explanation.description +")"}
            tmp_ret.push(obj);
            for (var tmp_cnt = 0; tmp_cnt < explanation.details.length; tmp_cnt++){
                detail = explanation.details[tmp_cnt];
                var val = extractFormula(detail, id)
        ...
        function parseExplanation(explanation){
            var expl = {};
            expl.total = {"boost":"total", "value":explanation.value};
            expl.formula = extractFormula(explanation, '#');
            var tmp_relevance = window.try_settings_relevance || options.relevance;
            tmp_relevance.fields_boosting.fields.forEach(function(field){
                var short_field = field.split("^")[0];
                expl[short_field] = {"boost":field, "value":findElemInExplanation(short_field, "field_boost", explanation)};
            });
            tmp_relevance.functions.forEach(function(fct){
                var decays = ['gauss', 'linear', 'exp'];
                var key = Object.keys(fct)[0];
                if (decays.includes(key)){
                    var field = Object.keys(fct[key])[0];
                    expl[field] = {"boost":field, "value":findElemInExplanation(field, "decay", explanation)};
                }
                if (key === "script_score"){
                    var field = fct[key].script;
                    expl[field] = {"boost":field, "value":findElemInExplanation(field, "script_score", explanation)};
                }
            });
            return expl;
        }
        ...
        var buildrecord = function(index) {
            ...
            for (var lineitem = 0; lineitem < display.length; lineitem++) {
                ...
                for (var object = 0; object < display[lineitem].length;
                    ...
                    if (thevalue !== undefined && thevalue.toString().length) {
        ...
        var resultsToJson = function(results, property, rel) {
            ...
            if (rel === 'AND' || rel === 'OR') {
                for (var element in results) {
                    jsonval.push({'text' : '<span class="facet_label_count">'+ results[element] + '</span>' + '<span class="facet_label_text i18n" i18n-variable="Facet_' + property.charAt(0).toUpperCase() + property.slice(1) + '_Item(' + element + ')" i18n-change="html">' + element + '</span>',
                                  'li_attr' : {
                                        'rel' : property,
                                        'class' : 'facetview_filterchoice leaf',
                                        'title' : element
        ...
        var updateJson = function(results, property, json, rel) {
            ...
            for (var element in json) {
                ...
                if (result_val === undefined) {
                    value.text = '<span class="facet_label_count">0</span>' + '<span class="facet_label_text i18n" i18n-variable="Facet_' + property.charAt(0).toUpperCase() + property.slice(1) + '_Item(' + text + ')" i18n-change="html">' + text +'</span>';
                } else {
                    value.text = '<span class="facet_label_count">' + result_val + '</span>' + '<span class="facet_label_text i18n" i18n-variable="Facet_' + property.charAt(0).toUpperCase() + property.slice(1) + '_Item(' + text + ')" i18n-change="html">' + text + '</span>';
                }
            }
            return new_json;
        };
        ...
        var addValuesToTree = function(fn_options) {
            ...
            if ($.inArray(current_facet.name, options.geofacets) !== -1){
                return;
            }
            var filteredRecords = fn_settings.records;
            if (($.inArray(facet, options.rangehistogramfacets) === -1) && ($.inArray(facet, options.aggregation_options) === -1)){
                filteredRecords = applyBlackAndWhiteListsOnFacetValues(fn_settings.records, facet_whitelist, facet_blacklist);
            }
...
/*            if (Object.keys(filteredRecords).length > fn_settings.size){
        ...
        var setFacetValues = function(sdata) {
            var facet = Object.keys(sdata.aggregations)[0];
            var tree = $('.facetview_tree[rel="' + facet + '"]');
            //todo, see if can be better, check parseresults
            var records = parsefacet(sdata.aggregations[facet]);
            var order = 'term';
            var doptions = [];
            ...
            for (var fct in facets) {
                ...
                if(curr_fct.field === facet) {
                    ...
                    fixed_order = curr_fct.fixed_order;
                    size = curr_fct.size;
        ...
        var correctFacetRenderer = function(data , trees, options){
            ...
            if(trees !== undefined && options !== undefined){
                correctHistogramFacetRenderer(data, trees, options);
            }
            $(".jstree-clicked").closest(".jstree-leaf").addClass("selected");
        };

        var correctHistogramFacetRenderer = function(data, trees, options){
        ...
        var refreshHistogram = function(data, el, rel){
            ...
            var datasetFilledMissing = formatRecords(valsobj.data("group_values"), histogram_config);
            ...
            if( sessionStorage.getItem(GROUPS_KEY + rel) !== null){
                ...
                datasetGroupByIntervalOLD = formatRecords(valsobj.data().records, histogram_config);
//                datasetGroupByIntervalOLD = populateEmptyGroupFromNewData(emptyDatasetGroupByInterval, datasetFilledMissing);
        ...
        var addDecayFunction = function(es_query, type, fct){
            ...
            if (tmp_relevance.score_mode !== undefined){
                es_query.query[options.query_type].score_mode=tmp_relevance.score_mode;
            }
            if (tmp_relevance.boost_mode !== undefined){
                es_query.query[options.query_type].boost_mode=tmp_relevance.boost_mode;
            }
            var new_func = {};
            new_func[type] = fct;
        ...
        var addDecayFunctions = function(es_query){
            if (settings_relevance_enabled){
                var tmp_relevance = window.try_settings_relevance || options.relevance;
                if (!$.isEmptyObject(tmp_relevance)){
                    if (tmp_relevance.functions !== undefined){
                        tmp_relevance.functions.forEach(function(value) {
                            addDecayFunction(es_query, Object.keys(value)[0], value[Object.keys(value)[0]]);
                        });
                    }
                    if (tmp_relevance.facet_decay_functions !== undefined){
                        var query_keys_list = [];
                        getAllKeys(query_keys_list, es_query);
                        $.each(tmp_relevance.facet_decay_functions, function(key, value) {
        ...
        var showTable = function(sdata) {
            ...
            for (var record_count = 0; record_count < data.records.length; record_count++){
                ...
                for (var field_count = 0; field_count < fields_in_use.length; field_count++){
                    ...
                    if ((fields_in_use[field_count].type === "date") && (fields_in_use[field_count].format !== undefined)){
                        if ($.datepicker) {
                            var tmp_date = new Date(record[field_name]);
                            if (!isNaN(tmp_date.getTime())){
                                record[field_name] = $.datepicker.formatDate('dd M yy', tmp_date);
                            }
                        }
                    }
                }
            ...
            if (options.pager_slider) {
                var metaTmpl = [
                    ...
                    'class="facetview_decrement eea_custom_btn" style="color:#333;float:left;',
                    'padding:0 40px 20px 20px;" href="{{from}}">&lt;</a> ',
                    '<span style="margin:30%;">{{from}} &ndash; {{to}} of ',
                    '{{total}}</span> ',
                    '<a alt="next" title="next" class="facetview_increment eea_custom_btn" ',
                ...
                var metaTmpl = [
                    ...
                    '<li class="prev"><a class="facetview_decrement eea_custom_btn" ',
                    'href="{{from}}">&lt&lt back</a></li> ',
                    '<li class="active"><a>Results {{from}} &ndash; {{to}} of ',
                    '{{total}}</a></li> ',
                    '<li class="next"><a class="facetview_increment eea_custom_btn" ',
                    'href="{{to}}">next &gt&gt</a></li> </ul> </div>'
                ].join('');
            }
...
/*            if (options.no_results_message) {
            ...
            if (data.found) {
                ...
                if (from < size) {
                    ...
                    $('.facetview_decrement', obj).closest("li").remove();
                }
                $('.facetview_increment', obj).bind('click', increment);
                if (data.found <= to) {
                    $('.facetview_increment', obj).html('..');
                    $('.facetview_increment', obj).closest("li").remove();
                }
            }
        ...
        var showFacets = function(sdata) {
            if (window.loading_results){
                setTimeout(function(){showFacets(sdata)}, 50)
            }
            else {
                _showFacets(sdata);
            }
        }
        var _showFacets = function(sdata) {
            options.rawdata = sdata;
            // get the data and parse from the es layout
            var fields_in_use = eea_mapping.listing;
            if (options.display_type === 'card'){
            ...
            for (var each = 0; each < options.facets.length; each++) {
                ...
                var records = data.aggregations[facet];
                var tree = $('.facetview_tree[rel="' + facet + '"]');
                var or_button = tree
                ...
                if( $.inArray(facet, options.rangehistogramfacets) !== -1 ){
                    ...
                    newQuery.aggs = {};
                    newQuery.aggs[facet] = esquery.aggs[facet];
                    ...
                    function parseRangeHistogramFacetValues(sdata){
                        ...
                        for ( var item in data.aggregations[tmp_facet] ) {
//                            if (!isNaN(item)){
                                rangevalues.push(item);
                                rangegroups[item] = data.aggregations[tmp_facet][item];
//                            }
                        }
...
/*                        if (header_class.indexOf('facetview_open') === -1 && header_class.indexOf('exact_helper') === -1 && $("#landing").css('display') !== "block") {
                            $header.click();
                        }*/ // don't open range histogram facets
                    ...
                    if (facet.indexOf("items_count") !== 0){
                        $.ajax({
                            ...
                            type: 'post',
                            url: options.search_url,
                            data: JSON.stringify({source: removedisplaytype(newQuery)}),
                            dataType: options.datatype,
                            success:function(d){
                        ...
                        items_count_sdata.aggregations[facet] = {};
                        for (var it_cnt = 1; it_cnt < 100; it_cnt ++){
                            items_count_sdata.aggregations[facet][it_cnt] = 1;
                        }
                ...
                if ($.inArray(facet, options.rangefacets) !== -1 ){
                    ...
                    if (facet.indexOf("items_count") !== 0){
                        ...
                        var items_count_sdata = {aggregations:{}};
                        items_count_sdata.aggregations[facet] = {};
                        for (var it_cnt = 1; it_cnt < 100; it_cnt ++){
                            items_count_sdata.aggregations[facet][it_cnt] = 1;
                        }
                ...
                if (or_button_rel === 'OR') {
                    ...
                    var esquery = JSON.parse(elasticsearchquery());
                    var filters;
                    if ((options.query_type === 'bool') || (esquery.query[options.query_type] === undefined)){
                        filters = esquery.query[options.query_type];
                    }
                    if ((esquery.query[options.query_type] !== undefined) && (options.query_type === 'function_score')){
                        filters = esquery.query[options.query_type].query.bool;
                    }
                    if ((!filters)||(!filters.filter)) {
                        addValuesToTree({
                            ...
                            fixed_order: current_filter.fixed_order,
                            size: current_filter.size,
                            autocomplete: current_filter.autocomplete
                        });

                    } else {
                        var newQuery = {'query': filters.must};
                        filters = filters.filter;
                        if (filters !== undefined){
                        if (('bool' in filters) && ('must' in filters.bool)){
                            var and = filters.bool.must;
                            var newand = [];
                            for (var aval in and) {
                        ...
                        if ('bool' in filters || 'and' in filters) {
                            ...
                            newQuery = {'query':{}};
                            newQuery.query['bool'] = {'must': tmp_newQuery.query, 'filter': filters};
//                            newQuery = addDecayFunctions(newQuery);
                        ...
                        if (facet.indexOf("items_count") !== 0){
                            ...
                            var items_count_sdata = {aggregations:{}};
                            items_count_sdata.aggregations[facet] = {buckets:[]};
                            for (var it_cnt = 1; it_cnt < 100; it_cnt ++){
                                items_count_sdata.aggregations[facet].buckets.push({term:it_cnt, count:1});
                            }
                    ...
                    if (jQuery.inArray(facet, options.aggregation_options) !== -1){
                        // if current facet is radio, "ignore" when fetching the counts for facet items
                        // remove the range from the query
                        var esquery = JSON.parse(elasticsearchquery());
                        var newQuery = {};
                        $.extend(newQuery, esquery);
                        newQuery.aggs = {};
                        newQuery.aggs[facet] = esquery.aggs[facet];
                        var ranges = findObjectsByKey(newQuery.query, 'range');
                        for (var r_cnt = 0; r_cnt < ranges.length; r_cnt++){
                            if (Object.keys(ranges[r_cnt])[0] === facet){
                                delete ranges[r_cnt][facet].from;
                                delete ranges[r_cnt][facet].to;
                            }
                        }
                        newQuery = JSON.stringify(newQuery);
                        filtersPopulatingCount++;
            ...
            if (options.isInitialSearch){
                ...
                if (facets_changed){
                    ...
                    if (window.settings_suggestions_enabled) {
                        $.fn.facetview.correct_and_dosearch(options)
                    } else {
                        dosearch()
                    }
                    options.isInitialSearch = false;
...
/*            if (options.no_results_message) {
        ...
        var buildqueryval = function() {
            ...
            if ((!$.isEmptyObject(tmp_relevance)) && (tmp_relevance.fields_boosting !== undefined)){
                qryval.fields = tmp_relevance.fields_boosting.fields;
            }
            return qryval;
        };
        ...
        var elasticsearchquery = function() {
            ...
            $('.facetview_filterselected', obj).each(function() {
                var facetname = $(this).attr("rel");
                var isAggregation = false;
                if ($.inArray(facetname, options.aggregation_options) !== -1){
                    isAggregation = true;
                }

                !bool ? bool = {'must': [] } : '';
                if ($(this).hasClass('facetview_facetrange') || isAggregation)  {
                    var addRange = true;
                    var rngs = {}
                    var rel = $(this).attr('rel');
                    if (isAggregation){
                        for (var of_cnt = 0; of_cnt < options.facets.length; of_cnt++){
                            if ($(this).attr('rel') === options.facets[of_cnt].field){
                                rel = of_cnt;
                                var val = $('#facetview_group_' +
                                    $(this).attr('rel')).find(".facetview_selection").text().trim();
                                for (var ao_cnt = 0; ao_cnt < options.facets[of_cnt].aggregation_options.length; ao_cnt++){
                                    var ao = options.facets[of_cnt].aggregation_options[ao_cnt]
                                    if (ao.label === val){
                                        if (ao.from !== undefined){
                                            rngs.from = ao.from;
                                        }
                                        if (ao.to !== undefined){
                                            rngs.to = ao.to;
                        ...
                        rngs.from = $('.facetview_lowrangeval_' +
                                $(this).attr('rel'), this).html(),
                        rngs.to = $('.facetview_highrangeval_' +
                                $(this).attr('rel'), this).html()
                    }
                    if (options.facets[rel].field.startsWith("items_count_")){
                        var facet_name = options.facets[rel].field.substring(12);
                    ...
                    if (addRange){
                        var rel = options.facets[rel].field;
                        var robj = {'range': {}};
                        robj.range[rel] = rngs;
                    ...
                    if ($(this).hasClass('facetview_logic_or')) {
                        ...
                        if ($.inArray(rel, seenor) === -1) {
                            ...
                            if (myfilter.bool.should.length === 0) {
                                ...
                                if (!filter) {
                                    filter = myfilter;
                                } else if (!filter.bool.must) {
                                    //two or filters on different relations
                                    filter = {'bool':{'must': [filter, myfilter]}};
                                } else {
                                    filter.bool.must.push(myfilter);
                                }
                            }
                        }
                    } else {
            ...
            if (filter) {
                ...
                qs['query'] = {};
                if (options.query_type === 'bool'){
                    qs['query'][options.query_type] = {
                        'must': tmp_qs['query'],
                        'filter': filter
                    }
                }
                if (options.query_type === 'function_score'){
                    qs['query'][options.query_type] = {
                                'query':{
                                    'bool':{
                                        'must': tmp_qs['query'],
            ...
            for (var item = 0; item < options.facets.length; item++) {
                ...
                delete fobj['fixed_order'];
                delete fobj['first_value'];
                delete fobj['last_value'];
                ...
                delete fobj['aggregation_options'];
                ...
                switch(fobj['order']) {
                    case "term":
                        ...
                        break;
                    case "reverse_term":
                        fobj['order'] = {"_key": "desc"}
                        break;
                    case "count":
                        fobj['order'] = {"_count": "asc"}
                        break;
                    case "reverse_count":
                        fobj['order'] = {"_count": "desc"}
                        break;
                }
                if ((jQuery.inArray(fobj.field, options.geofacets) === -1) && (jQuery.inArray(fobj.field, options.rangehistogramfacets) === -1) && (jQuery.inArray(fobj.field, options.aggregation_options) === -1) ){
                    qs['aggs'][fobj['field']] = {'terms': fobj};
                }
                if (jQuery.inArray(fobj.field, options.aggregation_options) !== -1){
                    qs['aggs'][fobj['field']] = {'range':{'field':fobj['field'], 'ranges':[]}};
                    for (var facet_cnt = 0; facet_cnt < options.facets.length; facet_cnt++){
                        if (options.facets[facet_cnt].field === fobj['field']){
                            var tmp_opts = options.facets[facet_cnt].aggregation_options
                            for (var options_cnt = 0; options_cnt < tmp_opts.length; options_cnt++){
                                var conf = {};
                                conf.key = tmp_opts[options_cnt].label;
                                if (tmp_opts[options_cnt].from !== undefined){
                                    conf.from = tmp_opts[options_cnt].from;
                                }
                                if (tmp_opts[options_cnt].to !== undefined){
                                    conf.to = tmp_opts[options_cnt].to;
                                }
                                qs['aggs'][fobj['field']]['range'].ranges.push(conf);
                            }
                        }
                    }
                }
                if (jQuery.inArray(fobj.field, options.rangehistogramfacets) !== -1){
//                    qs['aggs']['min_'  + fobj['field']] = {'min':{'field':fobj['field']}};
//                    qs['aggs']['max_'  + fobj['field']] = {'max':{'field':fobj['field']}};
                    qs['aggs'][fobj['field']] = {'range':{'field':fobj['field']}};
                    for (var facet_cnt = 0; facet_cnt < options.facets.length; facet_cnt++){
                        if (options.facets[facet_cnt].field === fobj['field']){
                            if (options.facets[facet_cnt].histogram_config.aggs_script !== undefined){
                                delete qs['aggs'][fobj['field']]['range'].field;
                                qs['aggs'][fobj['field']]['range'].script = options.facets[facet_cnt].histogram_config.aggs_script;
                            }
                            qs['aggs'][fobj['field']]['range'].ranges = options.facets[facet_cnt].histogram_config.ranges;
                        }
                    }
                }
                for (var ni; ni < options.nested.length; ni++) {
                    if (fobj['field'].indexOf(options.nested[i]) === 0) {
                         nested ?
                            qs['aggs'][fobj['field']]['scope'] =
                                options.nested[i] :
                            qs['aggs'][fobj['field']]['nested'] =
                                options.nested[i];
                    }
                }
            }
            jQuery.extend(true, qs['aggs'], options.extra_facets);
...
//qs['aggs'] = {}
        ...
        var dochangedisplay = function(event) {
            ...
            options.display_type = $this.attr('display-type');
        ...
        var dosuggestfacetvalues = function(tree_prop, request, response) {
            ...
            if (translation && translation.enabled){
                translatedValues = $.fn.i18nTranslateList(tree_prop, values);
            }
            else {
            ...
            for (var val_idx = 0; val_idx < values.length; val_idx++){
                if (translatedValues[val_idx].toLowerCase().indexOf(request.term.toLowerCase()) !== -1){
                    hints.push({term:translatedValues[val_idx], count:records[values[val_idx]], rel:tree_prop, originalTerm:values[val_idx]});
                }
            }
            hints = hints.sort(function(a,b){
                return a.term > b.term ? 1 : -1
            })
            response(hints.slice(0, 20));
        };

        $.fn.facetview.correct_search_term = function(search_term) {
            var dfrd = $.Deferred();
            var suggest_qry = {
                "size":0,
                "suggest": {}
            ...
            $.ajax({
                ...
                type: 'post',
                url: options.search_url,
                data: JSON.stringify({source: JSON.stringify(suggest_qry)}),
                dataType: options.datatype,
                success: function(data){
                    ...
                    if (window.settings_autocorrect_disable) {
                        dfrd.resolve(false)
                    }
                    else {
                        if (hasHints) {
                            dfrd.resolve(did_you_mean_parts)
                        } else {
                            dfrd.resolve(false)
                        }
                    }
        ...
        var dosuggest = function(request, response) {
            ...
            $.fn.facetview.correct_search_term(search_term)
            .done(function(correct_search_term) {
                ...
                if ((options.predefined_filters !== undefined) && (!jQuery.isEmptyObject(options.predefined_filters))){
                    autocomplete_qry.query = {"bool":{"must":options.predefined_filters}}
                }
                ...
                $.ajax({
                    ...
                    type: 'post',
                    url: options.search_url,
                    data: JSON.stringify({source: JSON.stringify(autocomplete_qry)}),
                    dataType: options.datatype,
                    success: function(data){
                        ...
                        if (window.settings_autocomplete_include_searchterm) {
        ...
        $.fn.facetview.test_enter = function(opts) {
          if (opts.which === 13){
            $("ul.ui-autocomplete").hide();
            $("body").next('inputs').focus();
          }
        }
        ...
        $.fn.facetview.correct_and_dosearch = function(opts) {
            ...
            if (search_term.length) {
                $('#facetview_freetext_clear').show();
            } else {
                $('#facetview_freetext_clear').hide();
            }
            $.ajax({
                contentType: "application/json",
                type: 'post',
        ...
        function getInitialValuesInStorage(query, data, callback){
            ...
            if(sessionStorage.getItem(INITIAL_RESULTS_KEY) !== null){
                ...
                $.ajax({
                    ...
                    type: 'post',
                    url: options.search_url,
                    data: JSON.stringify({source: qry }),
                    // processData: false,
                    dataType: options.datatype,
                    success:function(d){
/*                        $.each(d.aggregations, function(f_key, f_opts){
                            if (f_opts.buckets.length > 0){
                                var f_first = {};
                                var f_last = {};

                                var first_cnt = 0
                                var last_cnt = f_opts.buckets.length - 1

                                while (true){
                                    if (first_cnt >= last_cnt){
                                        break;
                                    }
                                    if (!isNaN(f_opts.buckets[first_cnt].key)){
                                        $.extend(f_first, f_opts.buckets[first_cnt]);
                                        break;
                                    }
                                    last_cnt--;
                                }
                                ...
                                while (true){
                                    ...
                                    if (!isNaN(f_opts.buckets[last_cnt].key)){
                                        $.extend(f_last, f_opts.buckets[last_cnt]);
                                        break;
                                    }
                                    last_cnt--;
                                }
                                f_opts.buckets =[f_first, f_last];
                            }
                        });*/
                        if(  d.aggregations !== undefined){
                            var aggs = Object.keys(d.aggregations);
                            for (var agg_idx = 0; agg_idx < aggs.length; agg_idx++){
                                if (jQuery.inArray(aggs[agg_idx], options.rangehistogramfacets) === -1){
                                    delete d.aggregations[aggs[agg_idx]];
                                }
                            }
                        }
...
//                window.history.pushState('', 'search', currurl);
            ...
            qrystr_noaggs = JSON.parse(qrystr)
            delete (qrystr_noaggs.aggs)
            qrystr_noaggs = JSON.stringify(qrystr_noaggs)
            if (options.enable_async_load) {
                window.loading_results = true;
                $.ajax({
                    type: 'post',
                    url: options.search_url,
                    data: JSON.stringify({source: removedisplaytype(qrystr_noaggs)}),
                    contentType: "application/json",
                    // processData: false,
                    dataType: options.datatype,
    ...
    //                    $(".facetview_metadata").text("Your search did not return any results").show();
            ...
            $.ajax({
                type: 'post',
                url: options.search_url,
                data: JSON.stringify({source: removedisplaytype(qrystr)}),
                contentType: "application/json",
                // processData: false,
                dataType: options.datatype,
                beforeSend : function( jqXHR, settings){
                ...
                success: function(d){
                    ...
                    // $(".facetview_metadata").show();
                    if (options.enable_async_load) {
                        getInitialValuesInStorage(removedisplaytype(qrystr), d, showFacets);
                    }
                    else {
                        getInitialValuesInStorage(removedisplaytype(qrystr), d, showresults);
                    }
...
//                    $(".facetview_metadata").text("Your search did not return any results").show();
        ...
        var parsesource = function() {
            ...
            function selectoptionvalue(optionfacet, from, to){
                var facet_id = optionfacet;
                var href = 'All';
                for (var i = 0; i < options.facets.length; i++){
                    if (options.facets[i].field === optionfacet){
                        for (var j = 0; j < options.facets[i].aggregation_options.length; j++){
                            oa = options.facets[i].aggregation_options[j];
                            if ((oa.from === from) && (oa.to === to)){
                                href = oa.label;
                            }
                        }
                    }
                ...
                var myobj = '<div class="facetview_selection facetview_selection5"> <a ' +
                        'class="facetview_filterselected facetview_clear btn';
                var operation = $('.facetview_or[href="' + rel + '"]', obj);
                var op_text = 'all';
                myobj = [myobj,
                ...
                if ($('div[id="facetview_group_' + relclean + '"]', obj).length) {
                    ...
                    var pobj = [
                            ...
                            relclean,
                            '" class="btn-group facetview_selected">',
                            '<h3 class="facetview_group_title facetview_group_title5">',
                            `<span class="i18n title" i18n-variable="Facet_Title(${title})" i18n-change="html" style="font-size: inherit">${title}</span>`,
                            '</h3>',
                            myobj,
                            '</div>'
            ...
            function selectrangevalues(rangefacet, from, to){
                ...
                var relclean = rel.replace(/\./gi, '_').replace(/\:/gi, '_');
                var myobj = '<div class="facetview_selection facetview_selection6">';
                var operation = $('.facetview_or[href="' + rel + '"]', obj);
                var op_text = 'all';
                myobj = [myobj,
                         '<span>',
                ...
                var pobj = [
                            ...
                            relclean,
                            '" class="btn-group facetview_selected ' + exact_helper_class + '">',
                            '<h3 class="facetview_group_title facetview_group_title6">',
                            `<span class="i18n title" i18n-variable="Facet_Title(${title})" i18n-change="html" style="font-size: inherit">${title}:</span>`,
                            '</h3>',
                            myobj,
                            '</div>'
            ...
            var query_to_parse = options.source.query;
            var and_terms = {};
            var or_terms = {};
            var query_string = '';
            var ranges = {};
            ...
            parseQueryTree(query_to_parse, []);
            $.each(and_terms, function(term_key, term_vals){
                for (var i = 0; i < term_vals.length; i++){
                    for (var j = 0; j < options.facets.length; j++){
                        if (options.facets[j].field === term_key){
                            clickfilterchoice(false, term_key, term_vals[i], false);
                        }
                    }
                }
            });
            $.each(or_terms, function(term_key, term_vals){
                for (var i = 0; i < term_vals.length; i++){
                    for (var j = 0; j < options.facets.length; j++){
                        if (options.facets[j].field === term_key){
                            clickfilterchoice(false, term_key, term_vals[i], true);
                        }
                    }
                }
            });
            options.q = query_string;
            $.each(ranges, function(range_key, range_val){
                for (var j = 0; j < options.facets.length; j++){
                    if (options.facets[j].field === range_key){
                        if (options.facets[j].type === "aggregation_options"){
                            selectoptionvalue(range_key, range_val.from, range_val.to);
                        }
                        else{
                            selectrangevalues(range_key, range_val.from, range_val.to);
                        }
                    }
                }
            });
if (false){ // disable but didn't delete for now
            if ('bool' in qrystr) {
                var qrys = [];
...
*/
        ...
        if (options.facets.length > 0 || options.static_filters.length > 0) {
            thefacetview = [
                ...
                '></div>',
                '</div>',
                '<div class="facets_footer_slot">',
                $(".facets_footer_slot_init").html(),
                '</div>',
                '</div><div class="span9" id="facetview_rightcol">',
                '<div class="facetedview_search">',
                '<span class="eea-icon eea-icon-search eea-icon-2x"></span>',
                '<input type="text" autofocus class="facetview_freetext span9 i18n" ',
                'style="display:block; margin-left:auto; margin-right: auto; ',
                'background:',
                options.searchbox_shade,
                '; name="q" value="" placeholder="Search term" i18n-variable="App_Search_Placeholder" i18n-change="placeholder" />',
                options.search_text_input_clear ? '<a id="facetview_freetext_clear" class="eea-icon eea-icon-times fa-2x"></a>':'',
                '</div>',
                ...
                '<div class="current-filters-collapsible eea-accordion-panel">',
                '<h2 class="filters-header__header">',
                    '<span class="facetview_arrow_right eea-icon eea-icon-right"></span>',
                    '<span i18n" i18n-variable="Search_Current_Filters_Header_Text" i18n-change="html">Current filters applied:</span>',
                    '<span class="">&nbsp</span>',
                    '<span class="filters-header__counter">10</span>',
                    '<a class="clear-all eea-icon eea-icon-eraser i18n-link eea-reset-filters" href="{{REFRESH}}"><span class="i18n" i18n-variable="Search_Reset_Filters_Link_Text" i18n-change="html">Reset filters</span></a>',
                    '<div style="clear:both"> </div>',
                '</h2>',
                '</div>',
/* disable global exact search
        ...
        if (window.settings_autocorrect_disable) {
            thefacetview += '<div class="facetview_top">' +
                ...
                '<span class="did_you_mean">Search instead for </span>' +
                '<a class="did_you_mean" href="#"></a>'+
                '<br/>' +
                '<span class="original_term">Showing results for </span>' +
        ...
        if (options.search_sortby.length >= 0) {
            thefacetview = [
                thefacetview,
                '<span class="orderby i18n" i18n-variable="Search_Order_Span_Text" i18n-change="html">Order &nbsp;</span>',
                '<select class="facetview_orderby">',
                '<option class="facetview_orderby_relevance i18n" value="" i18n-variable="Option(Relevance)" i18n-change="html">Relevance</option> '
            ].join('');
        ...
        if (options.display_type_options.length>1) {
            thefacetview += '<div class="facetview_display_type filter-by"><span class="i18n" i18n-variable="Search_Display_As_Span_Text" i18n-change="html">Display as </span>';
            for (var i=0; i<options.display_type_options.length; i++) {
                var dt = options.display_type_options[i];
                if (options.display_type===dt) {thefacetview += '<span class="i18n eea-icon selected ' + dt + '" title="' + dt + '" i18n-variable="Tooltip(' + dt + ')" i18n-change="title" display-type="' + dt + '"> </span>';}
                else {thefacetview += '<span class="i18n eea-icon ' + dt + '" title="' + dt + '" i18n-variable="Tooltip(' + dt + ')" i18n-change="title" display-type="' + dt + '"> </span>';}
            }
            thefacetview += '</div>';
        }
        ...
        thefacetview += '<a class="eea-icon eea-icon-file-excel-o eea_download_btn eea_custom_btn i18n" download_type="csv" href="tools/download" i18n-variable="Search_Download_CSV_Link_Text" i18n-change="html"><span>Download search results (CSV)</span></a>';
        thefacetview += '</div>';
        thefacetview += '<div class="facetview_metadata">';
        thefacetview += '</div></div></div></div></div>';
...
*/
                ...
                if (options.searchbox_class.length === 0) {
                    ...
                    $('.facetview_freetext', obj).bind(
                        'keyup',
                        $.fn.facetview.test_enter
                        );
                ...
                options.source || options.initialsearch ? $.fn.facetview.correct_and_dosearch(options) : '';
...
//        var search_query = decodeURIComponent(url.substr(8, url.length));
        ...
        try{
            var parsed = JSON.parse(search_query);
        }
        catch(err){
            return;
        }
        ...
        $('.facet-view-simple').append(landing);

        $.fn.facetview.provided_options.from_popstate = true;
        $.fn.facetview.provided_options.pushstate = false;
        $('.facet-view-simple').facetview($.fn.facetview.provided_options);
        setupHoverHandlers();

        $.fn.facetview.options.from_popstate = false;
        $.fn.facetview.options.pushstate = true;

if (false){ // disable original history functionality, don't delete for now
        var url = customGetUrl($.fn.facetview.options);
        var search_query = decodeURIComponent(url.split("?source=")[1]);
