...
*/
        ...
        function groupDatasetByIntervalWithExtremes(set, intervalInput, histogram_config) {
            ...
            } else if(histogram_config.columns_bucket_min !== undefined || histogram_config.columns_bucket_min !== null) {
                minArrRes = [{
                    x:set[0].x,
                    y: 0,
                    color: histogram_config.columns_min_color,
            ...
            } else if(histogram_config.columns_bucket_max !== undefined || histogram_config.columns_bucket_max !== null) {
                maxArrRes = [{
                    x: set_max,
                    y: 0,
                    color: histogram_config.columns_max_color,
                    ...
                    if (!item.from || !item.to && item.extreme === undefined) {
                        ...
                        if (item.to === undefined) {
                            if(arr[index + 1] !== undefined){
                                if(arr[index + 1].extreme !== undefined){
        ...
        function facetRangeHistogram(event,  step) {
            ...
            if( sessionStorage.getItem(GROUPS_KEY + rel) !== null){
                ...
                if(groupFromSession.length > 0){


                    $.each(groupFromSession, function(kg, group){
                        var yArr = [];

                        $.each(datasetFilledMissing,function(kdG, dataG){
                            if( groupFromSession[kg + 1] !== undefined){
                                if( dataG.x >= group.x && dataG.x < groupFromSession[kg + 1].x ){

                                    if( groupFromSession[kg].y !== dataG.y && dataG !== undefined){
                                        yArr.push(dataG.y);
                                    }
                                }
                            } else {
                        ...
                        groupFromSession[kg].y = yArr.reduce(function(prev, curr){
                            return prev + curr;
                        }, 0);

                    });

                    //datasetGroupByInterval = groupDatasetByIntervalWithExtremes(groupFromSession, intervalInput, histogram_config);
                    datasetGroupByInterval = groupFromSession;
            ...
            if($slider.length > 0){
                ...
                datasetGroupByInterval = datasetGroupByInterval.map(function(item){
                    var smaller = item.x > sliderV[ sliderH[1] ] ;
                    var larger = item.x < sliderV[ sliderH[0] ];
                    if(smaller || larger && item.color === undefined ){
                        item.color = "#EEEEEE";
                    }
                    return item;
                });

                //$slider.slider("values", [sliderH[0], sliderH[1]]);
            }

            var url_options = $.getUrlVars(options);

            if(url_options.source !== undefined && url_options.source.query.bool === undefined ){
        ...
        function renderSliderHistogram(rel, event, step, dataset, groupedDataset, intervalInput, sliderInterval, histogram_config,
            ...
            if(fromRefresh){
                groupedDataset = groupedDataset.filter(function(item){
                    var smaller = item.x < refreshSlider[1];
                    var larger = item.x >  refreshSlider[0];
                    return smaller && larger;
                });
            }
            ...
            if( sessionStorage.getItem("slider_" + rel) !== null ){
...
/*            if (Object.keys(filteredRecords).length > fn_settings.size){
        ...
        var correctHistrogramFacetRenderer = function(data, trees, options){
            ...
            $.each( filters,function(k, hfilter){
                ...
                rel = options.facets.findIndex(function(el, index, arr){
                ...
                if( el && $(el).length > 0){
                    step = options.step || 1;
                    target = that.closest(".facetview_filter").find(".facetview_showtree");
                    event = {
                        target : target,
        ...
        var refreshHistogram = function(data, el, rel){
            ...
            if( sessionStorage.getItem(GROUPS_KEY + rel) !== null){
                ...
                if(groupFromSession.length > 0){
                    ...
                    //datasetGroupByInterval = groupDatasetByIntervalWithExtremes(groupFromSession, intervalInput, histogram_config);
                    datasetGroupByIntervalOLD = groupFromSession;

                }

            } else {
                datasetGroupByIntervalOLD = groupDatasetByIntervalWithExtremes(datasetFilledMissing,
                    intervalInput,
                    histogram_config, true);
            }
            ...
            /*renderSliderHistogram(
                rel, event, intervalInput,
                datasetFilledMissing, datasetGroupByInterval,
                intervalInput, sliderInterval, histogram_config,
                true,
                [ sliderV[sliderH[0]], sliderV[sliderH[1]] ]
            );*/
            ...
            for (var each = 0; each < options.facets.length; each++) {
                ...
                if( $.inArray(facet, options.rangehistogramfacets) !== -1 ){
                    ...
                    function parseRangeHistogramFacetValues(sdata){
                        ...
                        if(tmp_facet==="Total_GHG_emissions_reductions_in_2020__kt_CO2eq_y_"){
                           // console.log("parseRangeHistogramFacetValues" ,sdata,tree);
                            shr += 1;

                            window.ady[shr] = [];
                            window.ady[shr].push({
                                sdata: sdata, tree: tree
                            });
                        ...
                        if(refreshOptions){
                            //console.log(tmp_facet);
                            //correctHistrogramFacetRenderer(tree, tmp_facet, refreshOptions.currentFilter, refreshOptions.filters, refreshOptions.rel);
                        }
                    }
