...
//        var addValuesToTree = function(orRel, facetName, tree, records, order, doptions) {
        var addValuesToTree = function(fn_options) {
            ...
            console.log(fn_options.facetName);
            console.log(fn_options.size);
            console.log(Object.keys(fn_options.records).length);
            if (Object.keys(fn_options.records).length > fn_options.size){
                return;
            }
            if ((!options.enable_exact) && (fn_options.facetName.startsWith("items_count_"))){
                return;
            }
            var values;
            if (options.hierarchy && options.hierarchy[fn_options.facetName].length > 0) {
                //set the values for the leaves
                for (var item in fn_options.records) {
                    var record = fn_options.records[item];
                    var inTree = fn_options.fn_options.tree.find('.jstree-leaf[title="' + item + '"]');

                    if (inTree.length > 0) {
                        fn_options.tree.jstree(true).rename_node(
                            inTree, '<span class="facet_label_count">' + record + '</span>' + '<span class="facet_label_text">' + item + '</span>');
                    }
                }

                //hide the ones with no values
                values = $('.jstree-node[rel="' + fn_options.facetName + '"]');
                for (var id = 0; id < values.length; id++) {
                    var value = values[id];
                    if (fn_options.records[value.title] === undefined) {
                        $(value).hide();
                    }
                }
            } else {
                var oldJson = fn_options.tree.jstree(true).get_json('#');
                fn_options.tree.jstree('destroy');
                if (oldJson.length === 0) {
                    createtreefromdata(
                        fn_options.tree,
                        fn_options.order,
                        fn_options.doptions,
                        resultsToJson(fn_options.records, fn_options.facetName, fn_options.orRel)
                    );
                } else {
                    createtreefromdata(
                        fn_options.tree,
                        fn_options.order,
                        fn_options.doptions,
                        updateJson(fn_options.records, fn_options.facetName, oldJson, fn_options.orRel)
                    );
                    var children = fn_options.tree.find('.jstree-leaf');
                    children.show();
                }
            }

            // Expand tree to desired height
            var ulTree = fn_options.tree.find('.jstree-container-ul')[0];
            var lineHeight = 24;
            var childCount = ulTree.childElementCount || 1;
            var prefHeight = childCount >= 10 ? 10 * lineHeight : childCount * lineHeight;
            fn_options.tree.height(prefHeight + 'px');
        };
