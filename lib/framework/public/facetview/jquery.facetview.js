...
*/
        ...
        //   ]
        function groupDatasetByInterval(dset, intervalInput,set_min, set_max, fromRefresh) {
        ...
        function groupDatasetByIntervalWithExtremes(set, intervalInput, histogram_config, fromRefresh) {
            var dec = (Number(intervalInput) === intervalInput && intervalInput % 1 !== 0) ?  2 : 0 ;
            var limit = dec === 2 ? 0.01 : 1;
            var set_min, set_max;
        ...
        function renderSliderHistogram(rel, event, step, dataset, groupedDataset, intervalInput, sliderInterval, histogram_config,
            ...
            if(maxval !== null && max !== maxval  ){
                ...
                lowrangeInput.val(values[values.length-1].toFixed(dec));
            } else {
...
/*            if (Object.keys(filteredRecords).length > fn_settings.size){
        ...
        var refreshHistogram = function(data, el, rel){
            ...
            if(rel === 7) console.log(sliderH);

            var filterDataset = datasetFilledMissing.filter(function(item){
                var smaller = item.x < sliderV[sliderH[1]];
                var larger = item.x > sliderV[sliderH[0]];
                return smaller && larger;
            });

            //if(rel === 7) console.log(datasetFilledMissing, filterDataset);

            //var datasetGroupByInterval = groupDatasetByIntervalWithExtremes(filterDataset, intervalInput, histogram_config);
            var datasetGroupByInterval = groupDatasetByIntervalWithExtremes(filterDataset, intervalInput, histogram_config);

            //if(rel === 7) console.log(datasetGroupByInterval.length, datasetGroupByIntervalO.length);
            //debugger;
            ...
            renderSliderHistogram(
                ...
                true, [sliderV[sliderH[0]], sliderV[sliderH[1]]]
            );
            ...
            for (var each = 0; each < options.facets.length; each++) {
                ...
                if( $.inArray(facet, options.rangehistogramfacets) !== -1 ){
                    ...
                    function parseRangeHistogramFacetValues(sdata){
                        ...
                        correctFacetRenderer(sdata, tree, options);
