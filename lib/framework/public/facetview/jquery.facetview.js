    ...
    $.fn.facetview = function(options) {
        ...
        var showfiltervalues = function(event) {
            ...
            if (these.hasClass('facetview_open')) {
                ...
                if ($.inArray( $(event.target).attr("id"), options.aggregation_options) !== -1){

                    facetAggregationOptions(event);

                    if ($(event.target).attr("id").indexOf("items_count_") !== 0){
                        these.parent().find(".facetview_optionscontainer").slideDown('fast');
                    }
...
//                var set_max_I = set_max + intervalInput;
        ...
        function facetAggregationOptions(event) {
            event.preventDefault();
            debugger;
// TODO Zotya
        }
