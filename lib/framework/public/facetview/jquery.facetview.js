...
*/
        ...
        var selectFromAutocomplete = function(event){
            var rel = $(event.target).closest("ul.ui-autocomplete").attr("rel");
            var title = $(event.target).closest("li.autocomplete_facet_value").find("span.facet_label_text").text();
...
/*                    if ( options.enable_rangeselect ) {
            ...
            for (var prop in options.hierarchy) {
                ...
                autocomplete_input.autocompleteForFacet({source:function(request, response){
                    var tree_prop = this.attr("rel");
                    dosuggestfacetvalues(tree_prop, request, response);}.bind(autocomplete_input)
                });
            }

            $('body').off('click', '.autocomplete_facet_value', selectFromAutocomplete);
            $('body').on('click', '.autocomplete_facet_value', selectFromAutocomplete);
