...
/*            if (Object.keys(filteredRecords).length > fn_settings.size){
        ...
        var addDecayFunction = function(es_query, type, fct){
            if (es_query.query.function_score.functions === undefined){
                es_query.query.function_score.functions = [];
            }
            var new_func = {};
            new_func[type] = fct;
            es_query.query.function_score.functions.push(new_func);
        }

        var addDecayFunctions = function(es_query){
            if (!$.isEmptyObject(options.relevance)){
                if (options.relevance.decay_function !== undefined){
                    $.each(options.relevance.decay_function, function(key, value) {
        ...
        var showresults = function(sdata) {
            ...
            for (var each = 0; each < options.facets.length; each++) {
                ...
                if (or_button_rel === 'OR') {
                    ...
                    var esquery = JSON.parse(elasticsearchquery());
                    var filters = esquery.query.function_score;
                    if (!filters) {
                        addValuesToTree({
                            orRel: 'OR',
                            facetName: facet,
                        ...
                        if ('bool' in filters || 'and' in filters) {
                            newQuery = {'query': {'function_score': {'query': newQuery.query, 'filter': filters}}};
...
//                    $(".facetview_metadata").text("Your search did not return any results").show();
        ...
        var parsesource = function() {
            ...
            if ('function_score' in qrystr) {
                var qrys = [];
                var flts = [];
                var or = false;
                var qryflt = qrystr.function_score;
