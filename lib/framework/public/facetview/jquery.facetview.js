...
// fixed the width of the autocomplete suggestion box
        ...
        var createfacetgeo = function (options){
            ...
            var geoselect = [
                '<div class="btn-group facetview_filter_options">',
                    '<button id="switch_distance_'+rel+'" type="button" class="btn btn-default map-selection-switcher-'+rel+'">',
                        'Distance <i class="icon-ok"></i>',
                    '</button>',
                    '<button id="switch_bounding_'+rel+'" type="button" class="btn btn-default map-selection-switcher-'+rel+'">',
                        'Bounding box <i class="icon-ok" style="display: none;"></i>',
                    '</button>',
                ...
                '<div class="clearfix">',
                    '<div id="geo-facet-tabs_'+rel+'">',
                        ...
                        '<div id="dialog_dist_'+rel+'" title="Select area">',
                            '<div id="map_dist_'+rel+'" class="google_map_dialog"></div>',
                        '</div>',
                        '<div id="dialog_bound_'+rel+'" title="Select area">',
                            '<div id="map_bound_'+rel+'" class="google_map_dialog"></div>',
                        '</div>',
                    '</div>',
                '</div></div>'
            ].join('');
            ...
            if(gmaps_loaded) {
                ...
                $('#switch_distance_' + rel).click(function() {
                    ...
                    if(!circle_dist) {
                        ...
                        distval = distval ? distval * 1000 : 200000;
                        latval = latval ? Math.round(latval * Math.pow(10, 6)) / Math.pow(10, 6) : 48.738525;
                        lngval = lngval ? Math.round(lngval * Math.pow(10, 6)) / Math.pow(10, 6) : 13.981955;
                        $(".facetview_distval_" + rel).val(distval / 1000);
                        $(".facetview_latval_" + rel).val(latval);
                        $(".facetview_lonval_" + rel).val(lngval);

                        circle_dist = new google.maps.Circle({
                            strokeColor: '#00446A',
                            strokeOpacity: 0.8,
                            strokeWeight: 2,
                            fillColor: '#006699',
                        ...
                        radius_changed_event = google.maps.event.addListener(circle_dist, 'radius_changed', function() {
                            // convert m to km
                            var distval = Math.round( circle_dist.getRadius() / 1000 );
                            $(".facetview_distval_" + rel).val(distval);
                        });

                        center_changed_event = google.maps.event.addListener(circle_dist, 'center_changed', function() {
                            var circle_center = circle_dist.getCenter().toJSON();
                ...
                $('#switch_bounding_' + rel).click(function() {
                    ...
                    if(!rect_bound) {
                        ...
                        // set default values if fields are empty
                        latval1 = latval1 ? Math.round(latval1 * Math.pow(10, 6)) / Math.pow(10, 6) : 50.738525;
                        lngval1 = lngval1 ? Math.round(lngval1 * Math.pow(10, 6)) / Math.pow(10, 6) : 9.981955;
                        latval2 = latval2 ? Math.round(latval2 * Math.pow(10, 6)) / Math.pow(10, 6) : 46.738525;
                        lngval2 = lngval2 ? Math.round(lngval2 * Math.pow(10, 6)) / Math.pow(10, 6) : 18.981955;
                        ...
                        rect_bound = new google.maps.Rectangle({
                            strokeColor: '#00446A',
                            strokeOpacity: 0.8,
                            strokeWeight: 2,
                            fillColor: '#006699',
                        ...
                        bounds_changed_event = google.maps.event.addListener(rect_bound, 'bounds_changed', function() {
                            var rect_bounds = rect_bound.getBounds().toJSON();
                            // get only 6 decimal coordinates
                ...
                $('.map-selection-switcher-' + rel).click(function(){
                    if(!$(this).find('i').is(":visible")) {
                        $('.map-selection-switcher-'+rel+' i').toggle();
                    }
                });

                // $(".facetview_dist_map_" + rel).click(function() {
                //     $( "#dialog_dist_" + rel).dialog( "open" );
                // });
                // $(".facetview_bound_map_" + rel).click(function() {
                //     $( "#dialog_bound_" + rel).dialog( "open" );
                // });
            } else {
                // if google maps lib is not loaded
            }
...
//                setfilterdescription("facetview_group_" + relclean, op_text);
        ...
        var addValuesToTree = function(orRel, facetName, tree, records, order, doptions) {
            ...
            var ulTree = tree.find('.jstree-container-ul')[0];
            var lineHeight = 24;
            var childCount = ulTree.childElementCount || 1;
            var prefHeight = childCount >= 10 ? 10 * lineHeight : childCount * lineHeight;
            tree.height(prefHeight + 'px');
        };
