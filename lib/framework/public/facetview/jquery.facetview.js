...
function customSetUrl(options, stateObj, page, url){
    ...
    if (getQueryLang(query) === default_language) {
        url = url.replace(`?lang=${getQueryLang(query)}`, `?`)
        ...
        var dofacetgeo = function(rel, dothesearch) {
            ...
            if ($('div[id="facetview_group_' + relclean + '"]', obj).length) {
                ...
                var pobj = [
                            ...
                            `<span class="i18n" i18n-variable="${title}" i18n-change="html" style="font-size: inherit">${title}</span>`,
                            '</h3>',
                            myobj,
                            '</div>'
                            ].join('');
...
/*            if(histoDataset.length === 0){
        ...
        function renderSliderHistogram(rel, event, step, dataset, groupedDataset, intervalInput, sliderInterval, histogram_config,
            ...
            var mu = muT === undefined ? "" : `<span class="um i18n" i18n-variable=${muT} i18n-change="html">${muT}</span>`;
            var rangeselect = [
                '<div id="facetview_rangeplaceholder_',
                rel,
...
/*                    if ( options.enable_rangeselect ) {
                    ...
                    exact_html =
                        '<div class="exact_facet_helper">'+
                            '<span class="eea-icon eea-icon-right eea-icon-question-circle eea-flexible-tooltip-top i18n"'+
                                'title="The *Exact* option will give you results that contain exclusivly the selected values and nothing else. This option may give you more precise results. Try it to find out." i18n-variable="tooltip_exact" i18n-change="title">'+
                            '</span>'+
                            '<span class="i18n" i18n-variable="Exact:" i18n-change="html">Exact:</span>'+
                    ...
                    autocomplete_html =
                        '<div class="facetview_filter_typeahead">'+
                            '<div>'+
                                ...
                                '<input class="i18n" i18n-variable="' + autocomplete_placeholder + '" i18n-change="placeholder" placeholder="' + autocomplete_placeholder + '" type="text" rel="' + prop +  '"/>' +
                            '</div>'+
                ...
                if (facet.type !== 'aggregation_options'){
                    ...
                    facet_options = [
                        ...
                        'style="display:none;">',
                        '<div class="facet_order facet_count_order ' + myOrder['count_class'] + '"><span class="i18n" i18n-variable="Count" i18n-change="html">Count</span><span class="eea-icon ' + myOrder['count_icon'] + '"></span></div>',
                        '<div class="facet_order facet_value_order ' + myOrder['term_class'] + '">' + '<span class="i18n" i18n-variable="' + short_name + '" i18n-change="html">' + short_name + '</span>' + '<span class="eea-icon ' + myOrder['term_icon'] + '"></span></div>',
                        ...
                        '<select class="facet_value_operator">',
                            '<option class="i18n" i18n-variable="Match all" i18n-change="html" value="and"' + option_rel_all_selected + '>Match all</option>',
                            '<option class="i18n" i18n-variable="Match any" i18n-change="html" value="or"' + option_rel_any_selected + '>Match any</option>',
                        '<select>',
                        '</div>',
                        '</div>'
                    ].join('')
...
*/
        ...
        if (options.facets.length > 0 || options.static_filters.length > 0) {
            thefacetview = [
                ...
                '; name="q" value="" placeholder="Search term" i18n-variable="Search term" i18n-change="placeholder" />',
                '</div>',
                ...
                '<h2 class="filters-header__header i18n" i18n-variable="Current filters" i18n-change="html">Current filters</h2>',
                '<a class="clear-all eea-icon eea-icon-eraser i18n-link" href="{{REFRESH}}"><span class="i18n" i18n-variable="Reset filters" i18n-change="html">Reset filters</span></a>',
/* disable global exact search
                exact_all_html,*/
        ...
        thefacetview += '<a class="eea_download_btn eea_download_stripe i18n" download_type="tsv" href="tools/download" i18n-variable="Download TSV" i18n-change="html">Download TSV</a>';
//        else {
            thefacetview += '<a class="eea_download_btn i18n" download_type="csv" href="tools/download" i18n-variable="Download CSV" i18n-change="html">Download CSV</a>';
//        }
        thefacetview += '</div>';
        ...
        if (options.search_sortby.length >= 0) {
            thefacetview = [
                thefacetview,
                '<span class="orderby i18n" i18n-variable="Order" i18n-change="html">Order &nbsp;</span>',
                '<select class="facetview_orderby">',
                '<option class="facetview_orderby_relevance i18n" value="" i18n-variable="Relevance" i18n-change="html">Relevance</option> '
            ].join('');
        ...
        if (options.display_type_options.length>1) {
            thefacetview += '<div class="facetview_display_type filter-by"><span class="i18n" i18n-variable="Display as" i18n-change="html">Display as </span>';
            for (var i=0; i<options.display_type_options.length; i++) {
                var dt = options.display_type_options[i];
                if (options.display_type===dt) {thefacetview += '<span class="i18n eea-icon selected ' + dt + '" title="' + dt + '" i18n-variable="tooltip_' + dt + '" i18n-change="title"> </span>';}
                else {thefacetview += '<span class="i18n eea-icon ' + dt + '" title="' + dt + '" i18n-variable="tooltip_' + dt + '" i18n-change="title"> </span>';}
            }
            thefacetview += '</div>';
        }
