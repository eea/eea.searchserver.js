...
//        var addValuesToTree = function(orRel, facetName, tree, records, order, doptions) {
        var addValuesToTree = function(fn_options) {
            ...
            console.log(fn_settings.facetName);
            console.log(fn_settings.size);
            console.log(Object.keys(fn_settings.records).length);
            if (Object.keys(fn_settings.records).length > fn_settings.size){
                return;
            }
            ...
            if (options.hierarchy && options.hierarchy[fn_settings.facetName].length > 0) {
                ...
                for (var item in fn_settings.records) {
                    var record = fn_settings.records[item];
                ...
                for (var id = 0; id < values.length; id++) {
                    var value = values[id];
                    if (fn_settings.records[value.title] === undefined) {
                        $(value).hide();
                    }
                }
            } else {
                ...
                if (oldJson.length === 0) {
                    createtreefromdata(
                        ...
                        resultsToJson(fn_settings.records, fn_settings.facetName, fn_settings.orRel)
                    );
                } else {
                    createtreefromdata(
                        fn_settings.tree,
                        fn_settings.order,
                        fn_settings.doptions,
                        updateJson(fn_settings.records, fn_settings.facetName, oldJson, fn_settings.orRel)
                    );
