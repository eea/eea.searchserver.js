...
*/
        ...
        var facetrangehistogram = function(event) {

            // TODO: when a facet range histogram is requested, should hide the facet
            //list from the menu should perhaps also remove any selections
            //already made on that facet
            event.preventDefault();
            var rel = $(event.target).attr('eea_rel');
            ...
            function startGoogleChart(elemId, dataFromFacet, facetOptions, rel){
                ...
                function drawChart() {
                    var dataSet = [['Year', 'Number']];
/*                    var yearBuckets = dataFromFacet.map(function(curr, index){
                        if(index % 2 === 0 && index % 5 === 0 ){
                            return index;
                        }
                    }).filter(Boolean);*/

                    $.each(dataFromFacet, function(key, value){
                    if (value !== undefined){
                        dataSet.push([key.toString(), value]);
                    }
                    });
                    dataFromFacet.forEach(function(index,curr){
/*                        yearBuckets.forEach(function(bucket, bindex){
                            if(curr > bucket && curr < (bucket + facetOptions.interval -1 )){
                                yearBuckets[bucket] = [];
                                yearBuckets[bucket].push({curr, index});
                            }
                        });
                        dataSet.push([index.toString(), curr]);*/
//                        dataSet.push(
                    });
//                    console.log(yearBuckets);
                    ...
                    var charW = $elem.width() + 6;

                    var facetChartOptions = facetOptions.chart_config;
                    ...
                    var chartOptions =  {
                        ...
                        chartArea : {backgroundColor: { stroke: '#fff'}},
                        vAxis : {baselineColor: '#fff', gridlineColor: '#fff', textPosition: 'none'},
                        hAxis : {baselineColor: '#fff', gridlineColor: '#fff', textPosition: 'none'},
                        isStacked: true
                    };
                    chartOptions = $.extend(true, chartOptions, facetChartOptions);
                    chartOptions.chartArea.width = chartOptions.width;
                    chartOptions.chartArea.height = chartOptions.height;
                    gchartdataTable = data;

                    var wrapper = new google.visualization.ChartWrapper({
                      chartType: facetChartOptions.chart_type || 'ColumnChart',
                        dataTable: data,
            ...
            startGoogleChart('facetview_rangehistogramplaceholder_container'+ rel, valsobj.data("group_values"), facetOptions, rel);
            ...
            $('#facetview_slider_' + rel, obj).slider({
                ...
                change: function( event, ui ) {
                    ...
                    if(gchart !== null && gchartdataTable !== null){
                       ...
                       // console.log(gchartdataTable.getNumberOfRows());
                       // gchart.draw(gchartDataTable,charOptions);
                    }
...
//                setfilterdescription("facetview_group_" + relclean, op_text);
        ...
        var parseresults = function(dataobj, cleanup) {
            ...
            for (var item in dataobj.aggregations) {
                resultobj.facets[item] = parsefacet(dataobj.aggregations[item]);
            }
            return resultobj;
        };
