...
(function($) {
    $.fn.facetview = function(options) {
        ...
        var orderBy_Cookie = getCookie('order');
...
/*            if (Object.keys(filteredRecords).length > fn_settings.size){
            ...
            if (options.hierarchy && options.hierarchy[fn_settings.facetName].length) {
                ...
                var oldJson = fn_settings.tree.jstree(true);
        ...
        function getCookie (cookieName) {
            ...
            for (var ci = 0; ci < cookies.length; ci++) {
                ...
                if (id === 0 && cookie.indexOf('=') === clen ) {
                    return decodeURIComponent(cookie.substring(clen + 1, cookie.length));
                }
            }
            return '';
        }
...
/*            options.paging.from = 0;
        ...
        $.fn.facetview.dosearch = function(opts) {
            ...
            if (window.history.pushState && options.pushstate) {
                ...
                options.pushstate_nr += 1;
                window.history.pushState(options.pushstate_nr, 'search', currurl);
            }
...
//                    $(".facetview_metadata").text("Your search did not return any results").show();
        ...
        var orderby = function(event) {
            ...
            setCookie('order', JSON.stringify(orderToStore), 365);
            options.paging.from = 0;
            dosearch();
        };
...
/* disable global exact search
        ...
        if (window.settings_es_debug_enabled){
            thefacetview += '<a class="eea_download_btn eea_download_stripe" download_type="csv" href="download">Download CSV</a>';
...
$(document).ready(function($){
    $(this).delegate(".eea_download_btn","click", function(event){
        ...
        if ($(this).attr("download_type") === "es_query"){
            var url_options = $.getUrlVars().source;
    ...
    var manipulate_expired_option = function(old_val, new_val) {
       var old_val_has_expired = old_val.indexOf('http://purl.org/dc/terms/expires');
       var new_val_has_expired = new_val.indexOf('http://purl.org/dc/terms/expires');
       if (old_val_has_expired === new_val_has_expired) {
           return false;
       }
       return true;
    };
    window.onpopstate = function(event) {
        var search_query = decodeURIComponent(window.location.search.substr(8, window.location.search.length));
        var old_query = $.fn.facetview.options.querystring;
        var url_vars = JSON.parse(search_query);
        var last_qry_vars = JSON.parse(old_query);

        // expired
        var expired_option = manipulate_expired_option(old_query, search_query);
        ...
        if (url_display !== lst_display) {
            $(".filter-by").find('.' + url_display).click();
        }

        // facet change
        var url_function_score = url_vars.query.function_score;
        var url_filters = url_function_score.filter.and || url_function_score.filter.bool.should;
        var url_queries = url_function_score.query.bool.must || url_function_score.query.bool.should;
        var function_score = last_qry_vars.query.function_score;
        var filters = function_score.filter.and || function_score.filter.bool.should;
        ...
        var $parent, $el;
        var current = {};
        var change_trigged = false;
        var $exact_facetviews = $(".facetview_exact");
        var $exact_facetviews;
        // exact change
        ...
        if ([-1, 1].indexOf(queries_change) !== -1 && url_filters.length - filters.length === 0) {
            queries_addition = queries_change === 1 ? true : false;
        ...
        // match change from any to all
        else if (Math.abs(url_queries.length - queries.length) !== 0 && Math.abs(url_filters.length - filters.length) !== 0) {
            // todo
        } else {
            for (var i = 0, length = url_queries.length; i < length; i++) {
                filter = url_queries[i];
                ...
                if (!current.hasOwnProperty(key)) {
                    current[key] = [];
                    $header.parent().find('.jstree-clicked').find('.facet_label_text').each(function(idx, el) { current[key].push(el.innerText)});
                }
