...
//        var search_query = decodeURIComponent(url.substr(8, url.length));
        ...
        for (var i = 0; i < $.fn.facetview.options.facets.length; i++){
            if (($.fn.facetview.options.facets[i].type === 'rangehistogram') || ($.fn.facetview.options.facets[i].type === 'range')){
                delete $.fn.facetview.options.facets[i].default_values;
            }
        }
...
//        $.fn.facetview.options.pushstate = false;
        $('.facet-view-simple').facetview($.fn.facetview.options);
        setupHoverHandlers();
