...
*/
        ...
        function createValuesObj(set) {
            ...
            for(var item in set) {
                result.push({y: parseFloat(item), x: set[item]});
            }
        ...
        function getFirstValue(value) {
            //if(value < 0) console.log(value.toFixed(3).substring(0,value.toString().length));
            //return parseFloat( value.toFixed(3));
            //console.log(typeof parseInt(value.toFixed(3).substring(0, value.toString().length-1)+0));
            return parseInt( value.toFixed(3).substring(0, value.toString().length-1) + 0);
        ...
        function groupDatasetByInterval(set, intervalInput) {
            console.log(set);
            var result = [{y: getFirstValue(set[0].y), x: set[0].x}]; // reset to 0 if for example interval starts at 1953

            var currentVal = result[0].y + intervalInput;
        ...
        function facetRangeHistogram(event,  step) {
            ...
            renderSliderHistogram(rel, event, step, datasetFilledMissing, intervalInput);
            setTimeout(function(){
        ...
        function renderHistogram(rel, dataset, intervalInput, event) {
            ...
            var datasetLength = dataset.length-1;
            svg.selectAll("rect")
                .data(dataset)
                .enter()
                .append("rect")
                .attr("x", function(d, i) {
                    var result = i * ((width / dataset.length-1) + padding);
                    return result;
                })
        ...
        function renderSliderHistogram(rel, event, step, dataset, intervalInput) {
            if ($("[id='facetview_rangeplaceholder_" + rel + "']").length > 0) {
                return;
            }
