...
*/
        ...
        function createValuesObj(set) {
            var result = [];
            for(var item in set) {
                result.push({y: parseInt(item), x: set[item]});
            }
            return result;
        }
        ...
        function getFistValue(value) {
            return parseInt(value.toFixed(0).substring(0, value.toString().length-1)+0)
        }
        ...
        function groupDatasetByInterval(set, intervalInput) {
            var result = [{y: getFistValue(set[0].y), x: set[0].x}]; // reset to 0 if for example decate starts at 1953
        ...
        function facetrangehistogram(event) {
            event.preventDefault();
            var rel = $(event.target).attr('eea_rel');
            var valsobj = $('.facetview_tree[rel="'+$(event.target).attr('id')+'"]', obj);
            var datasetInput = createValuesObj(valsobj.data("group_values"));
            var intervalInput = 10;
            var dataset = groupDatasetByInterval(datasetInput, intervalInput);
            var step = 1;

            renderSliderHistogram(rel, event, dataset[0].y, dataset[dataset.length-1].y, step);
            setTimeout(function(){
                renderHistogram(rel, dataset, intervalInput);
            })
        }
        function renderHistogram(rel, dataset, intervalInput) {
            ...
            svg.selectAll("rect")
                ...
                .append("rect")
                .attr("x", function(d, i) {
                    var result = i * (width / dataset.length-1);
                    return result;
                })
                .attr("y", function(d) {return height - yScale(d.x);})
                .attr("width", width / dataset.length-1 - padding)
                .attr("height", function(d) {return yScale(d.x);})
                .style("fill", barColor)
                .on("mouseover", function(d) {
                    tooltip.transition()
                        .duration(200);
                    tooltip.html( "<p><strong>" + d.y + " - " + (d.y + intervalInput - intervalInput/10) + "</strong></p><p><span>Number: " + d.x + "</span></p>")
        ...
        function fillMissingValues(set, step) {
            var result = [getFistValue(set[0])]; // reset to 0 if for example decate starts at 1953
            var currentVal = result[0] + step;

            for (var i = 1; i < set.length; i++) {
                if(set[i] <= currentVal) {
                    result.push(set[i]);
                } else if (set[i] <= currentVal + step){
                    result.push(set[i]);
                    currentVal += step;
                } else {
        ...
        function renderSliderHistogram(rel, event, firstVal, lastVal, step) {
            if ($("[id='facetview_rangeplaceholder_" + rel + "']").length > 0) {
                return;
            }
            var exact_helper_class = '';
            ...
            var rangeselect = [
                ...
                '<div id="facetview_rangehistogramplaceholder_container'+ rel +'" class="facetview_facetrange_histogram" style="margin:0 18px 3px 18px;">',
            ...
            var valuesInput = valsobj.data("values");
            var values = fillMissingValues(valuesInput, step);
            ...
            $('#facetview_slider_' + rel, obj).slider({
                ...
                values: [min, max],
                slide: function( event, ui ) {
                    var $elem = $("#facetview_slider_" + rel);
                    var width = $elem.width() + 6;
                    // console.log('event ', event, ' ui ', ui, ' values ', values, ' ui.values[0] ', ui.values[0], ' ui.values[1] ', ui.values[1])
                ...
                change: function( event, ui ) {
                    ...
                    $('#facetview_rangechoices_' + rel + ' .facetview_highrangeval_' + rel, obj).html( values[ ui.values[1] ] );
                    dofacetrange( rel );
                    if(gchart !== null && gchartdataTable !== null){
                        var rows = gchartdataTable.getNumberOfRows();
                        // console.log(rows);
                        gchartdataTable.removeRows(1, rows-1);
                    }
