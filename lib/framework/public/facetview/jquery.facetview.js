        ...
        var showfiltervalues = function(event) {
            ...
            if (these.hasClass('facetview_open')) {
                ...
                if ($.inArray( $(event.target).attr("id"), options.rangehistogramfacets) !== -1){
                    var intervalInput = options.intervalInput || 10;
...
*/
        ...
        //   ]
        function groupDatasetByInterval(set, intervalInput) {
            var result = []; // reset to 0 if for example interval starts at 1953

            var intervalRangeStart = getFirstValue(set[0].x, intervalInput);
            var intervalRangeEnd = getLastValue( set[ set.length-1 ].x , intervalInput);

            var i = intervalRangeStart;
            while( i <= intervalRangeEnd) {
                var filt = set.filter( function(item){
        ...
        function fillMissingValues(set, step, intervalInput, sliderInterval, groupedDataset) {
            var result = [];

            var intervalRangeStart = getFirstValue( set[0] , intervalInput);
            var intervalRangeEnd = getLastValue(set[set.length-1 ] , intervalInput);
        ...
        function facetRangeHistogram(event,  step) {
            ...
            smallArray[1010] = 1;
             ...
             var datasetFilledMissing = createValuesObj( smallArray);*/

            var datasetGroupByInterval = groupDatasetByInterval(datasetFilledMissing, intervalInput);

            renderSliderHistogram(rel, event, step, datasetFilledMissing, datasetGroupByInterval, intervalInput, sliderInterval);
            setTimeout(function(){
                // used setTimout to allow DOM to create the slider
                // we need it to determine the width of the histogram
                renderHistogram(rel, datasetGroupByInterval, intervalInput, event);

            })
        }

        function renderHistogram(rel, dataset, intervalInput, event) {
            ...
            var height = histogram_config.container_height || 150;
            var padding = 1;

            var barColor = histogram_config.bar_color || "#205c90";
            ...
            var yScale = d3.scaleLinear()
                ...
                .range([2, height]) // even a 0 will be represented so that we can have tooltip
                .nice();
            ...
            var barW = Math.round(width / datasetLength);
                ...
                .append("rect")
                    ...
                    .on("mouseover", function(d) {
                        ...
                        tooltip.html( function(){
                            return "<p><strong>" + d.y + " - " + (d.y + intervalInput - 1) + "</strong></p><p><span>Number: " + d.x + "</span></p>";
                        })
        ...
        function renderSliderHistogram(rel, event, step, dataset, groupedDataset, intervalInput, sliderInterval) {
            ...
            var rangeselect = [
                ...
                'class="facetview_lowrangeval_',
                rel,
                '">...</span><input name="lowrangeval_'+ rel +'" class="facetview_lowrangeval_'+ rel +'" style="width: 100px;display: none;" /> <small>to&nbsp;</small>',
                '<span class="facetview_highrangeval_',
                rel,
                '">...</span><input name="highrangeval_'+ rel +'" class="facetview_highrangeval_'+ rel +'" style="width: 100px;display: none;" /> </h3> ',
            ...
            var values = fillMissingValues(valsobj.data("values"), step, intervalInput, sliderInterval, groupedDataset);
            ...
            lowrangeText.on("click", function (e) {
                e.preventDefault();
                var $target = $(e.target);
                lowrangeInput.show().focus();
            ...
            lowrangeInput.on("blur", function(e){
                ...
                var newval = parseInt(lowrangeInput.val());

                var re = values.filter(function(item, index){
                ...
                $slider.slider("values", 0, sliderPos);

            });

            lowrangeInput.on('keyup', function (e) {
                var that = this;
                if (timeoutlow !== null) {
                    clearTimeout(timeoutlow);
                }
                timeoutlow = setTimeout(function () {
                    var re = null;
                    var newval = parseInt($(e.target).val());
                    if(newval < values[0] || newval > values[values.length-1]){
                        re = values[0];
                    } else {
                        re = values.filter(function(item){
                            var larger = newval > item;
                            var smaller = newval <= (item + intervalInput);
                            return smaller && larger;
                        })[0];
                    }
            ...
            highrangeText.on("click", function (e) {
                e.preventDefault();
                var $target = $(e.target);
                highrangeInput.show().focus();
            ...
            highrangeInput.on("blur", function(e){
                ...
                var newval = parseInt(highrangeInput.val());

                var re = values.filter(function(item, index){

                    var smaller = newval >= item;
                    var larger = newval < (item + intervalInput);
                    return smaller && larger;
                })[0];
                highrangeInput.val(re);

                var sliderPos = Object.keys(values).filter(function(key) {return values[key] === re})[0];
                $slider.slider("values", 1, sliderPos);

            });

            highrangeInput.on('keyup', function (e) {
                var that = this;
                if (timeouthigh !== null) {
                    clearTimeout(timeouthigh);
                }
                timeouthigh = setTimeout(function () {
                    var re = null;
                    var newval = parseInt($(e.target).val());
                    if(newval < values[0] || newval > values[values.length-1]){
