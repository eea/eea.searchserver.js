...
*/
        ...
        var clearfacetrange = function(event) {
            event.preventDefault();
            var rel = $(this).attr('rel');
            console.log("clearfacetrange", event.data);
        ...
        var clearfacetrangeHistogram = function(event, relP){
            console.log("clearfacetrangeHistogram", event.data);
            event.preventDefault();
            rel = $(this).attr('rel') || relP;
        ...
        //   ]
        function groupDatasetByInterval(dset, intervalInput,set_min, set_max) {
        ...
        function groupDatasetByIntervalWithExtremes(set, intervalInput, histogram_config) {
            ...
            var resultT = groupDatasetByInterval(dataset, intervalInput,set_min, set_max);
            ...
            var result = tempR.map(function(item, index, arr){
                ...
                if (item.x !== undefined) {
                    ...
                    if (!item.from || !item.to && item.color === undefined) {

                        if (item.from === undefined) {
                            item.from = arr[index - 1] !== undefined ? (item.x).toFixed(dec) :
                                set_min.toFixed(dec);
                            //item.from = item.x.toFixed(dec);
                        }
                        if (item.to === undefined) {
                            if(arr[index + 1] !== undefined){
                                if(arr[index + 1].color !== undefined){
        ...
        function fillMissingValues(intervalInput, sliderInterval, groupedDataset) {
            var result = [];
            groupedDataset = Array.from(new Set(groupedDataset));
        ...
        function fillMissingValuesWithExtremes(intervalInput, sliderInterval, groupedDataset, set, histogram_config){
            ...
            gDataset = Array.from(new Set(gDataset));
        ...
        function facetRangeHistogram(event,  step, rel) {

            if(event !== null){
                event.preventDefault();
            }

            var target = event.target || rel;

            var rel = $(target).attr('eea_rel');
            //console.log(rel,'.facetview_tree[rel="'+$(event.target).attr('id')+'"]', obj);

            var valsobj = $('.facetview_tree[rel="'+$(target).attr('id')+'"]', obj);
            ...
            var datasetGroupByInterval = groupDatasetByIntervalWithExtremes(datasetFilledMissing, intervalInput, histogram_config);


            renderSliderHistogram(rel, target, step, datasetFilledMissing, datasetGroupByInterval, intervalInput, sliderInterval, histogram_config);
            ...
            if(histogram_config.histogram === undefined || histogram_config.histogram !== false ){
                setTimeout(function(){
                    ...
                    renderHistogram(rel, datasetGroupByInterval, intervalInput, target, sliderInterval);
        ...
        function renderHistogram(rel, dataset, intervalInput, target, sliderInterval) {
            var histogramHeight = null;
        ...
        function renderSliderHistogram(rel, target, step, dataset, groupedDataset, intervalInput, sliderInterval, histogram_config) {

            function findCloseValue(newval, vals, type, histogram_config){
                var mp = {
                    min: 0,
                ...
                if(newval < vals[0] || newval >= vals[vals.length-1]){
                    re = mp[type];
                } else /*if(newval > histogram_config.columns_bucket_max)*/{

                    re = values.filter(function(item, index){
                        var hc = histogram_config;
                        var larger = newval >= item;
            ...
            var exact_helper_class = '';
            if ($(target).hasClass('exact_helper')){
                exact_helper_class = 'exact_helper';
            }
            ...
            $("[id='" + $(target).attr('id') + "']").after(rangeselect);
            ...
            lowrangeInput.on("blur",{ histogram_config: histogram_config },  function(e){
                ...
                if( !isInt($(e.target).val()) && !isFloat($(e.target).val()) || $(e.target).val() === "" ){
                    ...
                    if(newval >= values[parseInt($slider.slider("values",1))]){
                       ...
                       /* $('.facetview_filterselected[facettype="range"]', obj).unbind('click', clearfacetrangeHistogram);
                        $('.facetview_filterselected[facettype="range"]', obj).bind('click', clearfacetrangeHistogram);*/
                        setTimeout(function(){
                            dofacetrangeHistogram( rel,true, histogram_config, chosenValues );
                        }, 500);
                        detachedSlider = true;
...
/*            if (Object.keys(filteredRecords).length > fn_settings.size){
        ...
        var correctHistrogramFacetRenderer = function(tree, tmp_facet, filter, filters,rel){
            if(rel === undefined){
                return;
            }
            return ;
            var event = null;
            var $slider = $('#facetview_slider_' + rel);
            var el = $slider.parent().parent().children()[0];
            $(el).trigger("click").trigger("click");

            //var step = options.step;

            /*var otherHFilters = filters.filter(function(item){
               //return item
                return item.field !== filter.field && item.type === "rangehistogram";
            });*/
            //console.log(tree.data());

            //facetRangeHistogram(event, step, rel);
        ...
        var showresults = function(sdata,rel) {
            ...
            for (var each = 0; each < options.facets.length; each++) {
                ...
                if( $.inArray(facet, options.rangehistogramfacets) !== -1 ){
                    ...
                    function parseRangeHistogramFacetValues(sdata, refreshH, current_filter, filters, rel){
                        var data;
                        if (!sdata.skipParser){
                        ...
                        if(refreshH){
                            //correctHistrogramFacetRenderer(tree, tmp_facet, current_filter, filters, rel);
                        }
                    ...
                    if (facet.indexOf("items_count") !== 0){
                        $.ajax({
                            ...
                            success:parseRangeHistogramFacetValues
                                //parseRangeHistogramFacetValues(data, refresh, current_filter, options.facets, rel);
                        ...
                        items_count_sdata.refresh = refresh;
                        items_count_sdata.currentFilter = current_filter;
                        items_count_sdata.filters  = options.facets;
                        items_count_sdata.rel = rel;
...
//                window.history.pushState('', 'search', currurl);
            ...
            $.ajax({
                ...
                success:  function(data) {
                    if(opts !== undefined && opts.rel !== undefined){
                        showresults(data, opts.rel);
                    } else {
                        showresults(data);
                    }
