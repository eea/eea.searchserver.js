...
//                setfilterdescription("facetview_group_" + relclean, op_text);
        ...
        var clearfilter = function(event) {
            ...
            if (event.target.className.indexOf('eea-icon-times') === -1) {
                dosearch();
            }
...
/*            options.paging.from = 0;
        ...
        $.fn.facetview.dosearch = function(opts) {
            ...
            if (window.history.pushState && options.pushstate && !options.from_popstate) {
                ...
                var lastqry = JSON.parse(options.lastqrystr);
                delete(lastqry['facets']);
                // modify pushstate only if current url differs from previously saved url
                if (options.querystring !== JSON.stringify(lastqry)) {
