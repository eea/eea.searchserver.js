        ...
        var showfiltervalues = function(event) {
            ...
            if (these.hasClass('facetview_open')) {
                ...
                if ($.inArray( $(event.target).attr("id"), options.rangehistogramfacets) !== -1){
                    ...
                    facetRangeHistogram(event, intervalInput, step);
...
*/
        ...
        function groupDatasetByInterval(set, intervalInput) {
            var result = [{y: getFistValue(set[0].y), x: set[0].x}]; // reset to 0 if for example interval starts at 1953
        ...
        function facetRangeHistogram(event, intervalInput, step) {
            event.preventDefault();
            var rel = $(event.target).attr('eea_rel');
            var valsobj = $('.facetview_tree[rel="'+$(event.target).attr('id')+'"]', obj);
        ...
        function fillMissingValues(set, step, intervalInput) {
            var result = [getFistValue(set[0])]; // reset to 0 if for example interval starts at 1953
