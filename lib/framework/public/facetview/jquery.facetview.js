    ...
    if (!remove_landing) {
            ...
            for (var i = 0; i < facets.length; i++){
                if (facets[i].default_values){
                    default_values[facets[i].name] = facets[i].default_values.values;
                    default_values[facets[i].name].sort();
                }
            }
            $(".facetview_selected").each(function(id, element){
                ...
                selected_values[facet_name].sort();
            });
            isLanding = JSON.stringify(selected_values) === JSON.stringify(default_values)
//TODO: finalize the compare between selected_values and default_values
    }
