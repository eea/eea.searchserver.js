...
$(document).ready(function($){
    ...
    window.onpopstate = function(event) {
        ...
        var expired_state = $expired[0].checked;
        if (url_expired === -1 && !expired_state || url_expired && expired_state) {
            $expired.click();
        }
        ...
        if ([-1, 1].indexOf(queries_change) !== -1 && url_filters.length - filters.length === 0) {
            ...
            for (var i = 0, length = url_queries.length; i < length; i++) {
                ...
                if (key !== key_last_qry) {
                   ...
                   url_queries.length - queries.length === 0 && url_filters.length - filters.length !== 0) {
