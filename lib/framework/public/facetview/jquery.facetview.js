...
/*            if (Object.keys(filteredRecords).length > fn_settings.size){
        ...
        var correctHistrogramFacetRenderer = function(tree, tmp_facet, filter, filters){
            var event = null;
        ...
        var showresults = function(sdata) {
            ...
            for (var each = 0; each < options.facets.length; each++) {
                ...
                if( $.inArray(facet, options.rangehistogramfacets) !== -1 ){
                    ...
                    function parseRangeHistogramFacetValues(sdata, refreshH, current_filter, filters){
                        var data;
                        if (!sdata.skipParser){
                        ...
                        if(refreshH){
                            correctHistrogramFacetRenderer(tree, tmp_facet, current_filter, filters);
                        }
                    ...
                    if (facet.indexOf("items_count") !== 0){
                        $.ajax({
                            ...
                            success: function(data){
                                parseRangeHistogramFacetValues(data, refresh, current_filter, options.facets);
                                //parseRangeHistogramFacetValues(data);
                            }
                        });
                    }
                    else {
