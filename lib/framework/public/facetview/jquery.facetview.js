...
(function($) {
    $.fn.facetview = function(options) {
        ...
        var SLIDER_VALUES_KEY = "slider_values_";
        ...
        function facetRangeHistogram(event,  step) {
            ...
            if( checkInitialSearch()){
                Object.keys(sessionStorage).map(function(key){
                    if(key.indexOf(GROUPS_KEY + rel) !== -1 || key.indexOf(SLIDER_KEY + rel) !== -1){
                        sessionStorage.removeItem(key + rel);
                    }
                });
...
/*            if (Object.keys(filteredRecords).length > fn_settings.size){
        ...
        var refreshHistogram = function(data, el, rel){
            ...
            var sliderV = null;
            if(sessionStorage.getItem(SLIDER_KEY + rel) !== null && checkInitialSearch()){
            ...
            if( sessionStorage.getItem(GROUPS_KEY + rel) !== null){
                ...
                if(groupFromSession.length > 0){
                    ...
                    $.each(groupFromSession, function(kg, group){
                        ...
                        if(groupFromSession[kg].extreme !== undefined){
                            delete groupFromSession[kg].color;
                        }
                ...
                datasetGroupByIntervalOLD = groupDatasetByIntervalWithExtremes(datasetFilledMissing, intervalInput, histogram_config);
            }
...
/*            options.paging.from = 0;
        ...
        function getInitialValuesInStorage(query, data, callback){
            ...
            if(q.query.function_score && q.query.function_score.query.bool !== undefined){
                //if( q.query.function_score.query.bool.must !== undefined) delete q.query.function_score.query.bool.must;
                //if( q.query.function_score.filter.bool.should !== undefined) q.query.function_score.filter.bool.should = [];
            }
            //q.query = {"match_all":{}};
            ...
            $.ajax({
                ...
                success:function(d){
                    ...
                    setValueInSessionStorage("Initialresults", JSON.stringify(d));
                },
                error: function(e){
                    console.log(e);
                }
