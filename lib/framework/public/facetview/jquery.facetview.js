...
*/
        ...
        function getFirstValue(value, interval) {
            var r =  Math.floor( parseFloat(value) / parseFloat(interval)) * parseFloat(interval);
            return r;
        }

        function getLastValue(value, interval){
            return (Math.floor( value / interval) * interval + interval);
        }
        ...
        //   ]
        function groupDatasetByInterval(dset, intervalInput) {
            ...
            var intervalRangeStart = getFirstValue(dset[0].x, intervalInput);

            var intervalRangeEnd =  getLastValue( dset[ dset.length-1 ].x , intervalInput);

            var i = intervalRangeStart;

            while( i <= intervalRangeEnd) {
                var filt = dset.filter( function(item){
                    var max = i + intervalInput - 1;
                    return ( item.x >= i) && ( item.x <= max );
                });
            ...
            var res = result.map(function(item, index, arr){
                item.from = item.x.toFixed(dec);
                item.to = arr[index + 1] !== undefined ? ( arr[index+1].x - 1).toFixed(dec) : null;
                return item;
            });
        ...
        function groupDatasetByIntervalWithExtremes(set, intervalInput, histogram_config) {
            if( histogram_config.columns_bucket_min !== undefined ){
                if(histogram_config.columns_bucket_min === null ){
                    ...
                    var set_min = histogram_config.columns_bucket_min;
                }
            } else {
                var set_min = set[0].x;
            }
            ...
            if( histogram_config.columns_bucket_max !== undefined ){
                if(histogram_config.columns_bucket_max === null ){
                    var set_max = set[set.length-1].x;
                } else {
                    var set_max = histogram_config.columns_bucket_max;
                }
            } else {
                var set_max = set[set.length-1].x;
                var set_max = set[set.length-1].x;
            ...
            var result = groupDatasetByInterval(dataset, intervalInput);
            var minArrRes = [];
            var maxArrRes = [];
            ...
            if(minArr.length > 0){
                ...
                minArrRes = [{x:minFirst, y: minArrResT, color: histogram_config.columns_min_color }];
            }

            /*var minArrRes = (minArr || []).reduce(function(prev, curr, index, array){
                if(!prev){
                    prev = prev[0];
                }
                var sum = (parseInt(prev.y)|| 0) + parseInt(curr.y);
                var arr = [];
                return [{x:array[0].x, y: sum}];
            },[]);*/
            ...
            if(maxArr.length > 0){
                ...
                maxArrRes = [{x:maxLast , y: maxArrResT, color: histogram_config.columns_max_color }];
            }

            /*var maxArrRes = (maxArr || []).reduce(function(prev, curr, index, array){
                var sum = (parseInt(prev.y)|| 0) + parseInt(curr.y);
                var lastItem = array[array.length-1];
                return {x:lastItem.x, y: sum};
            },[]);*/

            //adding "to" and "from" to dataset
            var res = minArrRes.concat(result).concat(maxArrRes).map(function(item, index, arr){

                if(item.x !== undefined){
                    item.from = item.x.toFixed(dec);
                    item.to = arr[index + 1] !== undefined ? ( arr[index+1].x - 1).toFixed(dec) : (arr[index].x + intervalInput).toFixed(dec);
                    return item;
                }
            });
        ...
        function fillMissingValuesWithExtremes(intervalInput, sliderInterval, groupedDataset, histogram_config){
            ...
            if( histogram_config.columns_bucket_max !== undefined ){
                if(histogram_config.columns_bucket_max === null ){
                    var col_max = gDataset[gDataset.length-1];
                } else {
            ...
            if(datasetG.length > 0){
                var result = fillMissingValues(intervalInput, sliderInterval, datasetG);
            } else {
                var result = [];
            }
            ...
            if(sliderRatio > 1){
                ...
                minArrG = fillMissingValues(null, (tmpArrMin[1] - tmpArrMin[0])/sliderRatio, tmpArrMin).slice(0,-1).map(Math.floor);

                var tmpArrMax = [
                    datasetG[datasetG.length-1],
                    maxArrG[maxArrG.length-1]
                ];

                maxArrG = fillMissingValues(null, (tmpArrMax[1] - tmpArrMax[0])/sliderRatio, tmpArrMax).slice(1).map(Math.floor);

                var end = fillMissingValues(null, intervalInput/sliderRatio,
                    [ maxArrG[maxArrG.length-1], maxArrG[maxArrG.length-1]+parseFloat(intervalInput) ]);

                debugger;
            } else {
                var end = [
                    (parseInt(maxArrG[maxArrG.length-1]) + parseFloat(intervalInput))
                ];
            }
            /*if( minArrG.length % 2 !== 0 && minArrG.length > 1){
                var f = minArrG[minArrG.length-1];
                var l = datasetG[0];
                var diffMin = (l-f)/2;
                minArrG = minArrG.concat(diffMin);
            } else {
                //var f = minArrG[minArrG.length-1];
                //var l = datasetG[0];
                //var diff = l-f;
                ///var minArrG = minArrG.concat(diff);
            }*/


            /*if( maxArrG.length % 2 !== 0 && maxArrG.length > 1){
                var diff = maxArrG[maxArrG.length-1] + parseFloat(intervalInput/2);
                var end = [
                    diff,
                    (parseInt(maxArrG[maxArrG.length-1]) + parseFloat(intervalInput))
                ];
            ...
            var res = minArrG.concat(result).concat(maxArrG).concat( end );
            debugger;
            return res;
        }
        ...
        function facetRangeHistogram(event,  step) {
            ...
            var datasetGroupByInterval = null;

            if( histogram_config.columns_bucket_min !== undefined || histogram_config.columns_bucket_max !== undefined){
                datasetGroupByInterval = groupDatasetByIntervalWithExtremes(datasetFilledMissing, intervalInput, histogram_config);
            } else {
                datasetGroupByInterval = groupDatasetByInterval(datasetFilledMissing, intervalInput);
            }

            renderSliderHistogram(rel, event, step, datasetFilledMissing, datasetGroupByInterval, intervalInput, sliderInterval, histogram_config);
            ...
            if(histogram_config.histogram === undefined || histogram_config.histogram !== false ){
                setTimeout(function(){
                    ...
                    renderHistogram(rel, datasetGroupByInterval, intervalInput, event, sliderInterval);
        ...
        function renderHistogram(rel, dataset, intervalInput, event, sliderInterval) {
            var histogramHeight = null;

            var histoDataset = dataset.map(function(item) {
                return { x: item.y, y: item.x, from: item.from, to: item.to, color: item.color};
            });
            if( histoDataset[histoDataset.length - 1].x === 0 ){
                histoDataset.splice(-1,1);
            }
            ...
            var sliderWidth = $( "#facetview_slider_" + rel).width() - 2;
            var width = $( "#facetview_slider_" + rel).width()+2;
            ...
            if(histogram_config.ratio){
                ...
                histogramHeight = sliderWidth / 1.77;
            }
            ...
            var datasetLength = histoDataset.length;
            var ds = datasetLength - 1;

            var colWidth = Math.floor( ( (sliderWidth - ds ) / datasetLength) ) ;

            var $slider = $('#facetview_slider_' + rel);

            var sliderRange = ($slider.slider("option", "max") + 1);
            ...
            svg.selectAll("rect")
                ...
                .append("rect")
                    ...
                    .attr("width", function(a, i){
                        ...
                        return Math.floor( width / histoDataset.length-1 - padding);
                    })
                    ...
                    .on("mouseover", function(d) {
                        ...
                        tooltip.html( function(){
                                ...
                                return "<p><strong>" + from + " to " + to + "</strong></p><p><span>Results: " + d.x + "</span></p>";
                        })
                        .classed('hidden', false)
                        .style("left", function() {
                            return (d3.event.offsetX - $(this).width()) + "px"})
        ...
        function renderSliderHistogram(rel, event, step, dataset, groupedDataset, intervalInput, sliderInterval, histogram_config) {
            ...
            var rangeselect = [
                ...
                'class="facetview_lowrangeval_',
                rel,
                '">...</span><input name="lowrangeval_'+ rel +'" class="disabled facetview_lowrangeval_'+ rel +'"  /> <small>&nbsp;&nbsp;</small>',
                '<span class="facetview_highrangeval_',
                rel,
            ...
            if( histogram_config.columns_bucket_min || histogram_config.columns_bucket_max ){

                values = fillMissingValuesWithExtremes(intervalInput, sliderInterval, groupedDataset, histogram_config);

            } else {
                values = fillMissingValues(intervalInput, sliderInterval, groupedDataset);
            }
            ...
            lowrangeInput.on("blur", function(e){
                ...
                lowrangeText.html(re.toFixed(dec));
                lowrangeInput.val(re.toFixed(dec));
            ...
            highrangeInput.on("blur", function(e){
                ...
                if(re){
                    highrangeText.html(re.toFixed(dec));
                    highrangeInput.val(re.toFixed(dec));
                }
...
//                setfilterdescription("facetview_group_" + relclean, op_text);
        ...
        var parseresults = function(dataobj, cleanup) {
            var resultobj = {};
            resultobj.records = [];
            resultobj.start = '';
            resultobj.found = '';
                ...
                if( $.inArray(facet, options.rangehistogramfacets) !== -1 ){
                    ...
                    if (facet.indexOf("items_count") !== 0){
                        $.ajax({
                            ...
                            success: parseRangeHistogramFacetValues
                        });
                    }
                    else {
        ...
        if ([-1, 1, 0].indexOf(queries_change) !== -1 && url_filters.length - filters.length === 0 && !history_changed) {
            ...
            for (var i = 0, length = url_queries.length; i < length; i++) {
                ...
                if (key !== key_last_qry || is_range && queries_change === 0 || obj !== obj_last_qry && !is_range) {
                    ...
                    if (key.indexOf('items_count') !== -1) {
                        $exact_facetview = $exact_facetviews.filter("[href='" + key.substring(12, key.length) + "']");
