...
function display_results(display_fields, $results, widget_callback) {
    ...
    for (var i = 0; i < data.records.length; i++) {
        ...
        templateVariables.thumbUrl = templateVariables.url + image_scale;
        ...
        if (!(templateVariables.url.startsWith('http://www.eea.europa.eu'))) {
            templateVariables.thumbUrl = 'http://www.eea.europa.eu/portal_depiction/' + (templateVariables.contentType) + '/image_preview';
        }
        templateVariables.typeIcon = 'http://www.eea.europa.eu/portal_depiction/' + (templateVariables.contentType) + '/image_thumb';
            ...
            for (var rule_count = 0; rule_count < eea_mapping.types.images.rules.length; rule_count++){
                if (!found_rule_for_resource){
                    ...
                    if (found_rule_for_resource){
                        ...
                        templateVariables.thumbUrl = get_image("thumb_template");
                        templateVariables.typeIcon = get_image("icon_template");
                    }
                }
            }
        }
