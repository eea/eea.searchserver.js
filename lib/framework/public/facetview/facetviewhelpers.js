    ...
    return `<div class="eea-tile">
        <a class="eea-tileInner" title="${result.title}" href="${result.url}">
            ...
            <div class="eea-tileBody">
                <strong class="eea-tileType" style="background-image: url(${result.typeIcon})"><span class="eea-tileTypeIcon eea-tileType-${result.typeClass}" title="${result.type}"></span> ${result.type}</strong>
...
function get_widget_list(result) {
    return `<div class="tileItem visualIEFloatFix">
...
function display_results($results, widget_callback) {
    var data = $.fn.facetview.options.data;
    for (var i = 0; i < data.records.length; i++) {
        var element = data.records[i];
        var title = element['title'];
        var description = toEmpty(element['description']);
        var url = element['visualization'];
        var types = [];
        var type = 'Data Visualization';
        var typeClass = 'DavizVisualization';
        ...
        var result = {
            ...
            'url': url,
            'thumbUrl': url + '/image_mini',
            'type': type,
            'typeClass': typeClass,
            'typeIcon': 'http://www.eea.europa.eu/portal_depiction/' + (contentType) + '/image_icon',
            'topic': topics.join(', '),
            'datestamp': datestamp,
            'date': date,
        };
