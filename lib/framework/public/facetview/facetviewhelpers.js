...
function add_EEA_settings() {
    ...
    if (split.length === 2) {
        var html = [
            '<span class="default_pagination"><span class="results i18n" i18n-variable="Results" i18n-change="html">Results </span>',
            split[0],
...
function buildHeaders(mapping_listing) {
    ...
    for (var i = 0; i < mapping_listing.length; i++){
        ...
        if ((listing.title !== "") && (listing.title !== undefined)){
            ...
            headers += "<th class='i18n' i18n-variable='" + listing.title + "' i18n-change='title' title='" + listing.title + "'><div "+ header_width+"class='i18n' i18n-variable='Table(" + listing.title + ")' i18n-change='html' >" + listing.title + "</div></th>";
        }
    }
    return headers;
}
...
function addExactOnCurrentFacets(){
    ...
    $(".facetview_selected:visible").each(function(){
        ...
        $('[id="' + group_id + '"]').find(".facetview_group_title").find("span").not('.i18n').text(" (Match: " + text +")");
    });
    if (hasExact){
