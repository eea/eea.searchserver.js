...
function get_widget_card(result) {
    var template = '<div class="eea-tile"> \
        <a class="eea-tileInner" title="${result.title}" href="${result.url}"> \
            <div class="eea-tileThumb ${result.extra_css}" style="background-image: url(${result.thumbUrl})"><img src="${result.thumbUrl}" alt="${result.title}" /></div> \
...
debugger;
        ...
        var result = {
            ...
            '${result.url}': url,
            '${result.thumbUrl}': url + '/image_mini',
            '${result.type}': type,
            '${result.typeClass}': typeClass,
            '${result.typeIcon}': 'http://www.eea.europa.eu/portal_depiction/' + (contentType) + '/image_icon',
        ...
        if (url.indexOf('/themes/') <= 0) {
            result['${result.extra_css}'] = 'eea-tileThumb-default';
        }
