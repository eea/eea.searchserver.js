...
$(window).bind('post_search_callback', function(){
  if (_paq){
    ...
    if (results_found === 0){
      ...
      if (facets.length > 0){
        query_string += " " + JSON.stringify(facets);
      }
    }

    var categories = Object.keys(url_vars.facets).sort();
    _paq.push(['trackSiteSearch', query_string, categories, results_found]);
  }
});
...
function getUrlVars(){
  var newval;
  var params = {};
  var location_href = customGetUrl(window.location.href);
