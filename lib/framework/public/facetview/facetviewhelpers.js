...
function add_EEA_settings() {
    ...
    var $input_search_val = $(".facetview_freetext").val();
    if (split.length === 2) {
        var html = [
            '<span>Results ',
            split[0],
            ...
            '</strong></span>'].join('');
        if ($input_search_val) {
            html += ' results for "<strong class="facetedview_results_for">' + $input_search_val + '</strong>"';
        }
...
function addExactOnCurrentFacets(){
    ...
    $(".facetview_selected:visible").each(function(){
        ...
        $('[id="' + group_id + '"]').find(".facetview_group_title").find("span").text(" (in " + text +")");
    });
    if (hasExact){
...
function display_results(display_fields, $results, widget_callback) {
    ...
    for (var i = 0; i < data.records.length; i++) {
        ...
        templateVariables.description = description;
        templateVariables.url = url;
        templateVariables.datestamp = datestamp;
        templateVariables.topics = topics;
        templateVariables.types = types;
        ...
        templateVariables.date = $.datepicker.formatDate('dd M yy', new Date(templateVariables.datestamp));
        if (!(templateVariables.topics instanceof Array)) {
            templateVariables.topics = [templateVariables.topics];
        }
        if (!(templateVariables.types instanceof Array)) {
        ...
        var thumbUrl = templateVariables.url + image_scale;
        // if the result is from external sites just fall back to type depiction.
        if (!(templateVariables.url.startsWith('http://www.eea.europa.eu'))) {
            thumbUrl = 'http://www.eea.europa.eu/portal_depiction/' + (templateVariables.contentType) + '/image_preview';
        }
        var typeIconUrl = 'http://www.eea.europa.eu/portal_depiction/' + (templateVariables.contentType) + '/image_thumb';


        if ((eea_mapping.types.images !== undefined) && (eea_mapping.types.images.rules !== undefined)){
            if (eea_mapping.types.images.fallback_thumb !== undefined){
                thumbUrl = eea_mapping.types.images.fallback_thumb;
            }
            if (eea_mapping.types.images.fallback_icon !== undefined){
                typeIconUrl = eea_mapping.types.images.fallback_icon;
            }
            ...
            for (var rule_count = 0; rule_count < eea_mapping.types.images.rules.length; rule_count++){
                if (!found_rule_for_resource){
                    ...
                    var field_for_rule = element[rule.field];
                    var compare_result = false;
                    if (rule.rule === 'startsWith'){
                        compare_result = (element[rule.field].startsWith(rule.value));
                    }
                    if (rule.rule === 'contains'){
                        if (element[rule.field].indexOf(rule.value) !== -1){
                            compare_result = true;
                        }
                    }
                    ...
                    if (found_rule_for_resource){
                        function get_image(target){
                            var img_vars = {};
                            for (img_var_count = 0; img_var_count < rule[target].variables.length; img_var_count++){
                                var img_template = rule[target].template;
                                var img_var = rule[target].variables[img_var_count];
                                var img_var_value;
                        ...
                        thumbUrl = get_image("thumb_template");
                        typeIconUrl = get_image("icon_template");
                    }
                }
            }
        }
        ...
        var result = {
            '${result.title}': templateVariables.title,
            '${result.description}': templateVariables.description,
            '${result.url}': templateVariables.url,
            '${result.thumbUrl}': thumbUrl,
            '${result.type}': templateVariables.type,
            '${result.typeClass}': templateVariables.typeClass,
            '${result.typeIcon}': typeIconUrl,
            '${result.topic}': templateVariables.topics.join(', '),
            '${result.datestamp}': templateVariables.datestamp,
            '${result.date}': templateVariables.date
        };
