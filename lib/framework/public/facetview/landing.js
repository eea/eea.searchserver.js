...
//        var visibleValues = tree.find("li").filter(function(idx, el) { return el.style.display !== "none";});
        ...
        if (value === "min"){
            var tmp_facets = $('.facet-view-simple').facetview.options.facets;
            for (var i = 0; i < tmp_facets.length; i++){
                if (tmp_facets[i].field === facet){
                    retVal = tmp_facets[i].histogram_config.min_val;
                }
            }
        }
        if (value === "max"){
            ...
            for (var i = 0; i < tmp_facets.length; i++){
                if (tmp_facets[i].field === facet){
                    retVal = tmp_facets[i].histogram_config.max_val;
                }
            }
        }
        return retVal;
                ...
                if (valueSettingsForTile.type === "results"){
                    if (valueSettingsForTile.method !== undefined){
                        ...
                        if (valueSettingsForTile.value === "rows"){
                            if (!$.fn.facetview.options.landing_rawdata_started){
                                var sort_query = JSON.parse($.fn.facetview.options.querystring);
                                if (valueSettingsForTile.sort !== undefined){
                                    sort_query.sort = valueSettingsForTile.sort;
                                }
                                delete sort_query.display_type;
                                delete sort_query.ignore_landing;
                                $.fn.facetview.options.landing_rawdata_started = true;
                                $.ajax({
                                    type: 'get',
                                    url: $.fn.facetview.options.search_url,
                                    data: {source: JSON.stringify(sort_query)},
                                    dataType: $.fn.facetview.options.datatype,
                                    success: function(sdata){
                                        $.fn.facetview.options.landing_rawdata = sdata;
                                    }
                                });
                            }
                            if ($.fn.facetview.options.landing_rawdata){
                                $("[class='"+valueSettingsForTile.name+"']").empty();
                                var results = $.fn.facetview.options.landing_rawdata.hits.hits;
                                value = "";
