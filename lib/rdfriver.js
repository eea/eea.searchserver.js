...
const get_missing_docs = async(river_config) => {
    return new Promise(async (resolve, reject) => {
        ...
        idx_helpers.imalive();
        let docs_in_semantic = await get_rdfs_from_endpoint(tmp_river_config);
        idx_helpers.imalive();
...
const index_river = async(river_config, bulk_size) => {
    return new Promise(async (resolve, reject) => {
        const missing_docs = await get_missing_docs(river_config);
        let rdfs_list = await get_rdfs_from_endpoint(river_config);
