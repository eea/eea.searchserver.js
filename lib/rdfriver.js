        ...
        try{
            ...
            for (let i = 0; i < bulk.length; i++){
                ...
                try{
                    ...
                    if (tmp_stats.clusters.errors[river_config.river.cluster_id] === undefined){
                        tmp_stats.clusters.errors[river_config.river.cluster_id] = {};
                    }
                    if (tmp_stats.clusters.errors[river_config.river.cluster_id].sparql === undefined){
                        tmp_stats.clusters.errors[river_config.river.cluster_id].sparql = [];
                    }
                    tmp_stats.clusters.errors[river_config.river.cluster_id].sparql.push(err.rdfs[0].resource.value);
...
//TODO index each doc one by one
        }
    })
}

const get_missing_docs = async(options) => {
    return new Promise(async (resolve, reject) => {
        try{
            const river_config = options.river_config;
            ...
            const docs_in_elastic = await idx_helpers.get_docs_from_index(tmp_river_config.river.cluster_id);
            idx_helpers.imalive();

            let missing_docs = [];
            for (let i = 0; i < docs_in_semantic.length; i++){
                if (docs_in_elastic.indexOf(docs_in_semantic[i].resource.value) === -1){
                    missing_docs.push(docs_in_semantic[i]);
                }
            }

            resolve(missing_docs);
        }
        catch(err){
            resolve(err);
        }
...
const index_river = async(river_config, bulk_size) => {
    return new Promise(async (resolve, reject) => {
        try{
            let total_indexed = 0;
            let indexed = 0;
            const missing_docs = await test_and_execute(river_config.index, get_missing_docs, {river_config: river_config});

            let rdfs_list = await test_and_execute(river_config.index, get_rdfs_from_endpoint, {river_config: river_config});

            let clean_rdfs_list = rdfs_list.map(result => result.resource.value)
            for (let i = 0; i < missing_docs.length; i++){
                if (clean_rdfs_list.indexOf(missing_docs[i].resource.value) === -1){
                    rdfs_list.push(missing_docs[i]);
                }
            }
            let tmp_bulk = [];
            ...
            tmp_stats.clusters.errors[river_config.river.cluster_id].sparql.push(e);
            await idx_helpers.set_status(tmp_stats);
