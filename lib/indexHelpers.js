...
const reindex = async() => {
    return new Promise(async (resolve, reject) => {
        ...
        const qs = {
            "source": {
                "index": options.elastic.index + "_prod"
            },
            "dest": {
        ...
        console.log("Starting reindex from " + options.elastic.index + "_prod to " + options.elastic.index + "_latest");
        console.log("Requests per second:", rps);
        new esAPI(options)
            .POST('_reindex?requests_per_second=' + rps, qs, function(err, statusCode, header, body){
                if (err === null){
                    console.log("Reindex from " + options.elastic.index + "_prod to " + options.elastic.index + "_latest was successful");
                    resolve(true);
                }
                else {
                    console.log("Error: Reindex from " + options.elastic.index + "_prod to " + options.elastic.index + "_latest failed");
                    console.log(err)
                    resolve(false)
                }
            })
...
const delete_doc = async(id) => {
    return new Promise(async (resolve, reject) => {
        ...
        var docPath = elastic.index + "_latest/" + elastic.type + '/';
