...
const create_index = async(settings) => {
    ...
    if (analyzer_status){
        ...
        await index_helpers.set_status({status:'indexing', indexing_started_at:new Date()})
        ...
        while(true){
            ...
            while(true){
                ...
                if (data.has_elements){
                    is_correct_alias = await index_helpers.test_alias(indexName)
                    if (is_correct_alias){
...
const api_status = async(settings) => {
    ...
    if (status.status === 'indexing'){
        const last_timestamp = new Date(status.timestamp);
        status.elapsed_time_since_last_status_change = deltatohuman(last_timestamp, current_date);
        const start_timestamp = new Date(status.indexing_started_at);
        status.elapsed_time_since_indexing_started = deltatohuman(start_timestamp, current_date);
    }
