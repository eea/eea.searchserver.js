...
const create_index = async(settings) => {
    ...
    if (analyzer_status){
        ...
        if (data_source.type.toLowerCase() === 'url'){
            ...
            let url_to_index = data_source.configuration.url;
            ...
            if ((data_source.configuration.count_url !== undefined) && (settings.update_from_url === undefined)){
                let count_data = '';
                const count_req = request({
                    uri:data_source.configuration.count_url
                })
