...
const INDEXING_INACTIVITY_DELTA = 5
...
const create_index = async(settings) => {
    ...
    if (latest_status){
        if (latest_status.status === 'indexing'){
            ...
            const diff_mins = Math.round(((diff % 86400000) % 3600000) / 60000);
    ...
    if (analyzer_status){
        ...
        if ("subtype" in data_source.configuration) {
            if (data_source.configuration.subtype.toLowerCase() === 'discomap'){
                discobulk = data_source.configuration.discomap_bulksize;
                first_index = 0;
                last_index = discobulk;
                var discomap_url = data_source.configuration.url;
        ...
        if (indexing_cancelled){
            ...
            prod_index_docs = index_stat.body.indices[prod_stats.index_name].primaries.docs.count;
            latest_index_docs = index_stat.body.indices[latest_stats.index_name].primaries.docs.count;
