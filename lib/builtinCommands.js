...
const readerFile = require('eea-searchserver').readerFile;
...
const create_index = async(settings) => {
    ...
    if (analyzer_status){
        ...
        if (data_source.type === 'file'){
            const file_reader = new readerFile(data_source.configuration);
            while(true){
                let data = await file_reader.read_bulk();
console.log(data.counter);
console.log(data.bulk_counter);
//                await index_helpers.index_bulk(rows);
