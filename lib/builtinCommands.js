...
const create_index = async(settings) => {
    ...
    if (analyzer_status){
//zotya        const indexName = await index_helpers.create_empty_index(analyzers.analyzers);
const indexName = 'tmp'
        if (settings.API_callback !== undefined){
...
//zotya        await index_helpers.set_status({status:'indexing', indexing_started_at:new Date(), index_name: indexName})
...
//zotya            await index_helpers.reindex()
...
//zotya            const docs_in_semantic = await sparqlHelpers.get_docs_from_semantic(data_source.configuration.endpoint, rivers_from_config);
//zotya            console.log("Number of documents in semantic: " + docs_in_semantic.length);
            ...
            for (let i = 0; i < rivers_from_config.length; i++){
                const river = rivers_from_config[i];
                await index_helpers.create_river({endpoint:data_source.configuration.endpoint, river:river});
            }
        }
        else {
            while(true){
