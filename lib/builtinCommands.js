...
function polyfillMapping(mapping){ // make old mapping.jsons compatible with new elasticsearch
    delete (mapping['_id']);
    mapping['rdf_lastRefreshed'] = {"type": "text"};
    ...
    for (var i = 0; i < fields.length; i++){
        ...
        if (field.type === 'text'){
            ...
            if (field.copy_to.indexOf('all_fields_for_freetext') === -1){
...
const get_index_files = async(settings) => {
    ...
    console.log('dirname now', __dirname);
    var charFilter = path.resolve(__dirname, 'builtinCharFilters.json');
    console.log('analyzer path', charFilter);
    var charFilters = require(charFilter);

    console.log('dirname now', __dirname);
    var tokenizer = path.resolve(__dirname, 'builtinTokenizer.json');
    console.log('analyzer path', tokenizer);
    var tokenizers = require(tokenizer);
    ...
    if ((settings.extraTokenizers !== undefined) && (settings.extraTokenizers !== '')){
        var settingsTokenizers = require(path.resolve(settings.app_dir,settings.extraTokenizers));
        tokenizers = _.extend(tokenizers, settingsTokenizers);
    }

    if ((settings.extraCharFilters !== undefined) && (settings.extraCharFilters !== '')){
        var settingsCharFilters = require(path.resolve(settings.app_dir,settings.extraCharFilters));
        charFilters = _.extend(charFilters, settingsCharFilters);
    }
    ...
    if (es_version === 'es7'){
      mappings.mappings.properties = mapping;
    }
...
const execScript = async(options) => {
    return new Promise(async (resolve, reject) => {
        try {
            const { script } = options;
            const child = child_process.spawn('python', [script]);
...
const create_index = async(settings) => {
    ...
    if (analyzer_status){
        ...
        const tmp_prod_stats = await index_helpers.get_status("prod");
        if (skip_reindex){
          await index_helpers.switch_latest_alias(settings);
        }
        let indexName = '';
        if (tmp_prod_stats !== undefined){
            indexName = tmp_prod_stats.index_name;
        }
        if (!skip_reindex){
            indexName = await index_helpers.create_empty_index(idx_create_options);
        }
        if (settings.API_callback !== undefined){
        ...
        if (data_source.type.toLowerCase() === 'file'){
            if (data_source.configuration.create_file_script){
              await execScript({script:data_source.configuration.create_file_script});
            }
            ...
            if (settings.update_from_url !== undefined){
                ...
                let source_file = path.join(settings.config_dir, data_source.configuration.file);
                if (data_source.configuration.create_file_script){
                  source_file = data_source.configuration.file;
                }
        ...
        if ((data_source.type.toLowerCase() === 'url') || (force_update_from_url)){
            ...
            if ((has_count) && (settings.update_from_url === undefined)){
                ...
                .on('end', async() => {
                    ...
                    console.log("Expected nr. of documents:", data_to_read)
                    let status = await index_helpers.get_status();
        ...
        if (data_source.type === 'rdfriver'){
            while(true){
                ...
                if ((!data_source.configuration.fullreindex) && (!reindex_clusters)){
                    try{
                        if (!is_first){
...
reindex_cluster = async(settings, clusters) => {
    if (clusters !== undefined){
        ...
        console.log("No cluster specified");
    }
}
