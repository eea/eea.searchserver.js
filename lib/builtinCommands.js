...
const create_index = async(settings) => {
    ...
    let indexing_start_time = Math.floor(Date.now() / 1000)
    ...
    if (analyzer_status){
        ...
        if (data_source.type === 'rdfriver'){
            ...
            for (let i = 0; i < rivers_from_config.length; i++){
                ...
                await index_helpers.create_river({endpoint:data_source.configuration.endpoint, river:river, start_time: updated_at});
            }
        }
        else {
            while(true){
                ...
                if ("subtype" in data_source.configuration) {
                    if (data_source.configuration.subtype.toLowerCase() === 'discomap'){
                        ...
                        let status = await index_helpers.get_status();
                        if (status.nr_of_docs_to_index > total_counter + 4) {
                            has_more_parts = true;
                        }
                        else {
                            has_more_parts = false;
