...
function getMapping(settings, mappingCallback) {
  var mapping = {};
  var mappingFile = path.join(settings.app_dir,settings.dataMapping);
  if (fs.existsSync(mappingFile) &&
    ...
    var query = fs.readFileSync(path.join(settings.app_dir,settings.indexingQuery), 'utf8') + '\nLIMIT 10';
...
function getIndexFiles(settings, indexCallback) {
  ...
  if ((settings.extraAnalyzers !== undefined) && (settings.extraAnalyzers !== '')){
    var settingsAnalyzers = require(path.join(settings.app_dir,settings.extraAnalyzers));
    analyzers = extend(analyzers, settingsAnalyzers);
  }

  var filters = require(path.join(__dirname, 'builtinFilters.json'));
  if ((settings.filterAnalyzers !== undefined) && (settings.filterAnalyzers.length !== 0)){
    var settingsFilters = require(path.join(settings.app_dir,settings.filterAnalyzers));
