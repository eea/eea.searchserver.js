...
module.exports = function(callback) {
    ...
    function getExternalTemplate(options, fileName, tagName, strip_templates){
        var externalTemplateRequest = tool.request(options, function (res) {
            ...
            res.on('end', function () {
                if (res.statusCode >= 200 && res.statusCode < 300 && data.length) {
                    if(strip_templates === true) {
                        dataCheerio = cheerio.load(data);
                        data = dataCheerio(tagName).html();
                    }
    ...
    function downloadExternalTemplates(){
        var strip_templates = nconf.get("external_templates:strip_templates")
        if(typeof(strip_templates) == 'undefined') {
        ...
        getExternalTemplate(head_options, headFile, 'head', strip_templates);
        ...
        getExternalTemplate(header_options, headerFile, 'body', strip_templates);
        ...
        getExternalTemplate(footer_options, footerFile, 'body', strip_templates);
    }
