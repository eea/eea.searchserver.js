...
var setCachedValues = function(name, ttl, values){
    ...
    elastic.index = elastic.index + "_cache";
...
var getCachedValues = function(name, ttl, callback) {
    ...
    var nconf = require('nconf')
    var elastic = nconf.get()['elastic'];
    var indexed_url = 'http://' + encodeURIComponent(elastic.rwuser) + ":" + encodeURIComponent(elastic.rwpass) + "@" + elastic.host + ':' + elastic.port + elastic.path + elastic.index + '_cache/' + 'cache/' + name;
    var res;
    var cached_value = {};
    try {
